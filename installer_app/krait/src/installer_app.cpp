...
static const int QMMF_RESTART_WAIT_TIME_FOR_AUDIO_INIT = 2;
static const int KILL_RUNNING_TINYPLAY_APLAY_TIMEOUT = 5;
...
static const string default_stream_port("8900");
static const string otacheck_state_path = "/dev/shm/otacheck_state.txt";
...
const char *const kInstallerConnIndicatorFile     = "/dev/shm/installer_app_connected";
...
static const string queue_name = "q_ins_resp";
static const string installer_queue = "installer_queue";

static const string gpio_path = "/sys/class/gpio/gpio171";
static const string gpio_level_info_file = gpio_path + "/value";
...
static uint64_t button_ack_time = 0;
static const string BUTTON_ACK_TIME_FILE = "/dev/shm/button_ack_time";

static const string vehicle_config_path = "/data/nd_files/config/vbus_installer.ini";
...
static const string EXT_CAM_Q_NAME = "EXT_CAM";
...
const char* kInstallerScanIndicatorFile = "/dev/shm/installer_scan_ongoing";
bool nd_central_led_ack_received = false;
bool led_blinking_active = false;
int led_blinking_duration = 5; // in secondsf
...
#ifdef KRAIT
/* Stop apm service to make sure ignition and device does not go OFF when
 * connected to installer app
 */
bool stop_apm_service(){

    static const string stop_apm = "systemctl stop apm.service";

    int ret =0;

    ret = system_execute ("stopping apm service", stop_apm);

    sleep (UNIT_TIME);

    if (ret !=0){
       LOG_E(TAG, "apm service is not stopped after client connected\n");
       return false;
    }
    return true;
}

bool kill_running_tinyplay(void *cmd){

    int ret = system_execute("KILL_TINYPLAY", "killall -KILL tinyplay");
    return ((0 == ret) ? true : false );
}

bool kill_running_aplay(void *cmd){

    int ret = system_execute("KILL_APLAY", "killall -KILL aplay");
    return ((0 == ret) ? true : false );
}

/* This is a audio reinitialization function to reinit audio in case of qmmf-server restart.
   This function is written from "start_stream_playback()" API in nd_tinyalsa.cpp. 
   TODO: Move this function to nd_tinyalsa.cpp in nd_core_utils. */
static void audio_reinit() {

    task_result_t timed_task_result;
    
    LOG_I(TAG, "Starting audio reinitialization process");
    
    // Kill tinyplay
    timed_task_result = nd_timed_task(kill_running_tinyplay, KILL_RUNNING_TINYPLAY_APLAY_TIMEOUT, NULL, "kill tinyplay");
    if (TASK_SUCCESS != timed_task_result) {
        LOG_E(TAG, "nd_timed_task failed for kill_tinyplay with status : %d", (int)timed_task_result);
    }

    // Kill aplay
    timed_task_result = nd_timed_task(kill_running_aplay, KILL_RUNNING_TINYPLAY_APLAY_TIMEOUT, NULL, "kill aplay");
    if (TASK_SUCCESS != timed_task_result) {
        LOG_E(TAG, "nd_timed_task failed for kill_aplay with status : %d", (int)timed_task_result);
    }
    ...
    // Verify speaker init
    if (!nd_device_obj->verify_speaker_init(vol_params)) {
        LOG_E(TAG, "User set values of audio_init are not matching with get values, So again doing audio_init");
        nd_device_obj->speaker_init(vol_params); // Re-initialize audio settings
    }

    // Verify mic init
    if (!nd_device_obj->verify_mic_init()) {
        LOG_E(TAG, "User set values of mic_init are not matching with get values, So again doing mic_init");
        nd_device_obj->mic_init(); // Re-initialize mic settings
    }
    
    LOG_I(TAG, "Audio reinitialization process completed");
}

bool restart_qmmf_service(){

   int ret=0;

   static const string stop_qmmf_server = "systemctl stop qmmf-server.service";
   static const string start_qmmf_server = "systemctl start qmmf-server.service";

   ret = system_execute("stop qmmf-server service", stop_qmmf_server);

   if (ret !=0){
       LOG_E(TAG, "qmmf-server service is not stopped\n");
       return false;
   }

   sleep (NVCAMERA_RESTART_TIME);

   ret = system_execute("start_qmmf-server", start_qmmf_server);
   if (ret !=0){
      LOG_E(TAG, "qmmf-server service is not started\n");
       return false;
   }
   sleep (UNIT_TIME);
...
bool read_configurations(){
    ...
    if (!bagheera_config_parser.getParseStatus()){
        ...
        stream_port = bagheera_config_parser.getConfig("INSTALLER_APP", "stream_port", default_stream_port);
    }
...
bool accept_client_connection(int &listenfd, int &fd, string timeout) {
    ...
    string_to_int64(timeout, timeout_num);

    // using non blocking select api
    bool res = timeout_setting_for_socket(listenfd, timeout);

    ifstream button_ack_time_file(BUTTON_ACK_TIME_FILE.c_str());
    int64_t button_ack_time = 0;
    if(button_ack_time_file.good()) {
        button_ack_time_file >> button_ack_time;
    }
    LOG_I(TAG, "button_ack_time = %lld", button_ack_time);

    // time diff in seconds
    int64_t time_diff = 1 + ((button_ack_time + timeout_num*1000) - get_system_monotonic_time()) / 1000;

    if(time_diff > 2) {
        stringstream ss;
        ss.clear();
        ss << time_diff;
        res = timeout_setting_for_socket(listenfd, ss.str());
    }

    file_delete(BUTTON_ACK_TIME_FILE);
    if (res){
...
#ifdef KRAIT
bool qmmf_start_streaming(int cam_num)
{
    ...
    ret = qmmf_create_video_track(qmmf_handle, cam_num, session_id, track_id);
    if(ret != 0)
    {
...
bool qmmf_stop_streaming(int cam_num) {
...
ss << "v4l2src device=/dev/video0 ! video/x-raw , width=(int)1920 , height=(int)1080 , format=(string)UYVY ! nvvidconv ! video/x-raw(memory:NVMM) , width=(int)640 , height=(int)360 , format=(string)NV12 ! omxh265enc bitrate=300000 ! video/x-h265,stream-format=(string)byte-stream ! rtph265pay pt=96 ! udpsink host=" << str << " port=4000";
    ...
    ss << "nvcamerasrc sensor-id=" << camera_no_string << " ! video/x-raw(memory:NVMM),width=640, height=360, framerate=30/1, format=NV12 ! omxh265enc bitrate=300000 ! video/x-h265,stream-format=(string)byte-stream ! rtph265pay pt=96 !  udpsink host=" << str  <<  " port=4000";
    ...
    LOG_I(TAG, "starting %d camera !!!!!!", argument);
    int cam_num = 0;
    if(argument == OUT_CAM) {
       cam_num = 0;
    }
    else if(argument == BACK_CAM) {
       cam_num = 1;
    }
    bool ret = qmmf_start_streaming(cam_num);
    if(ret != true) {
        return response_streaming(argument, sequence_no, false);
    }
#endif
...
bool stop_streaming(int cam_num){
        ...
        else if(cam_num == BACK_CAM) {
            cam_num_local = 1;
        }

           qmmf_stop_streaming(cam_num_local);
#endif
           ss.str("");
           sleep (UNIT_TIME);
...
bool check_sim_status(){
...
#ifndef KRAIT
    static const string lte_gps_sample_app = "lte_gps_sample_app 'AT!ICCID?' > lte.txt";
#else
    static const string lte_gps_sample_app = "lte_gps_sample_app 'AT!ICCID?' > lte.txt";
#endif

   ret = system_execute("lte test", lte_gps_sample_app);
   if (ret !=0){
      LOG_E(TAG, "system call for sim test failed\n");
      return false;
   }

    FILE* in_file = fopen("lte.txt", "r");
    if (!in_file){
       LOG_E(TAG, "can not read lte file\n");
       return false;
    }
   ...
   while (fscanf(in_file, "%s", str1)!=EOF){
         ...
         if(strcmp(str1, "OK") == 0){
            fclose (in_file);
            system_execute("remove_lte.txt", "rm lte.txt");
            return true;
         }
   }
   fclose (in_file);
...
bool stop_fan_service(){

    int ret = 0;
    static const string fan_stop = "systemctl stop fan_control.service";
    ret = system_execute("stop fan service", fan_stop);
    if (ret == -1){
       LOG_E(TAG, "did not stop fan service");
       return false;
    }
...
{
    std::ifstream gpio_val(gpio_level_info_file.c_str());
    char value;
...
bool get_ignition_status(bool &ignition_status) 
{
    ...
    int msp_curr_ign_stat = 0;
    bool ret = false;
    ret = nd_msp_read_curr_int_status(&msp_curr_ign_stat);
        if(true != ret) {
        //TBD: print error
        LOG_E(TAG, "Unable to read current msp status\n");
        return false;
    }
    if(msp_curr_ign_stat == 1) {
        LOG_I(TAG, "Ignition status is ON\n");
        ignition_status = true;
    }
    else {
        LOG_I(TAG, "Ignition status is OFF\n");
        ignition_status = false;
    }
    return true;
}
...
bool check_internet() {
...
#ifdef KRAIT
    // Using Generic API for querying internet status through ping  on ipv4/ipv6 DNS IP's
    return check_internet_exist();
    //string cmd = "busybox ping -c 2 8.8.8.8 | tail -1 | grep \"round-trip\" | awk \'{print $4}\' | cut -d \'/\' -f 2 2>&1";
#else
    string cmd = "ping -c 1 -w 1 8.8.8.8 | tail -1 | awk '{print $4}' | cut -d '/' -f 2";
    string resp = "";
    LOG_I(TAG, "ping cmd: %s", cmd.c_str());
    if(!system_execute_with_resp("PING_INT_CHECK", cmd, resp)) {
    ...
    if (pos != string::npos) {
        resp.assign (resp, 0, pos);
    }

    long int res_time = strtol (resp.c_str(), NULL, 10);
...
bool get_signal_data(signal_data_s &signal_data) {
    ...
    string cmd = "lte_gps_sample_app 'AT!GSTATUS?'";
    string gst_status_resp;

    if(!system_execute_with_resp("GSTATUS", cmd, gst_status_resp)) {
    ...
    size_t start_pos = 0;
    size_t find_itr = 0;
    LOG_I(TAG, "Status output:\n");
    while((find_itr = gst_status_resp.find("\n", start_pos)) != std::string::npos) {
        LOG_I(TAG, "%s", gst_status_resp.substr(start_pos, find_itr - start_pos).c_str());
        start_pos = find_itr + 1;
    }

    if (std::string::npos == gst_status_resp.find("!GSTATUS:")) {
        // module error
        LOG_E(TAG, "Module error?");
        return false;
    }

    ATGStatus_map status_map = get_parsed_atgstatus_map(gst_status_resp);
...
bool check_imei_no(string &imei_data){
     ...
     bool res =false;
     string resp = "";
     string cmd = "read_kraitinfo_emmc | grep 'IMEI serial_num'";
     LOG_D(TAG, "IMEI cmd is %s", cmd.c_str());

     res = system_execute_with_resp("IMEI no check command", cmd.c_str(), resp);
     if(!res){
        LOG_E(TAG, "imei no check command failed");
        return false;
     }
     //resp = "IMEI serial_num:123456789012345"; //test
     LOG_I(TAG, "resposne of imei check cmd : %s", resp.c_str());
     size_t pos = resp.find (':');

    if(pos == string::npos) {
        LOG_E(TAG,"imei cmd parsing failed");
        return false;
     }
     LOG_D(TAG, ": detected at position %d in string : %s", pos, resp.c_str());
     resp.assign (resp, pos+1, resp.length() - (pos+1));
     imei_data = resp;
...
bool check_iccid_num(string &iccid_num){
     ...
     bool res =false;
     string resp = "";
     string cmd = "lte_gps_sample_app 'at!iccid?' | grep '!ICCID:'";
     LOG_D(TAG, "iccid cmd is %s", cmd.c_str());

     res = system_execute_with_resp("iccid no check command", cmd.c_str(), resp);
     if(!res){
        LOG_E(TAG, "iccid no check command failed");
        return false;
     }
     LOG_I(TAG, "resposne of iccid check cmd : %s", resp.c_str());
//     resp = "!ICCID: 89882390000160175587"; //test
     size_t pos = resp.find (':');

    if(pos == string::npos) {
        LOG_E(TAG,"iccid cmd parsing failed");
        return false;
     }
     LOG_D(TAG, ": detected at position %d in string : %s", pos, resp.c_str());
     resp.assign (resp, pos+1, resp.length() - (pos+1));
...
bool check_gps_satellite_info(){
     ...
     string cmd = "";
     string result = "";
     cmd = "lte_gps_sample_app 'AT!GPSSATINFO?'";
     bool ret = system_execute_with_resp("gps satellite info command", cmd.c_str(), result);
     if (!ret)
        LOG_E(TAG, "getting satellite info failed");

     LOG_I(TAG, "%s", result.c_str());
}
...
bool diagnostic_device_check(diagnostic_msg_t *diagnostic_msg) {
    ...
    std::vector<std::string> diagnostic_params = {"simcard", "internet", "ignition", "gpsInfo", "signal", "imei", "iccid", "sdcard", "fan"};
    ...
    if (config_json) {
        ...
        LOG_E(TAG, "failed to load config_json %s:", diagnostic_msg->config_json);
    }
...
bool service_related_activities(){
   ...
   gst_init(NULL, NULL);
#else
    // In case of KRAIT, restart qmmf-server service
    LOG_I(TAG, "Restarting qmmf-server service\n");
    res = restart_qmmf_service();

    if (!res){
       LOG_E(TAG, "qmmf-server service not restarted\n");
       return false;
    } else {
       LOG_I(TAG, "qmmf-server service restarted successfully\n");
    }

   // Reinitialize audio settings after QMMF restart as audio settings are lost after QMMF restart.
   sleep (QMMF_RESTART_WAIT_TIME_FOR_AUDIO_INIT);
   audio_reinit();
...
#ifdef KRAIT
    res = stop_apm_service();
    if (!res){
       LOG_E(TAG, "apm service not stopped\n");
       return false;
    }
    ...
    {
       sleep(UNIT_TIME);
       stop_service("fan_control.service");
    }
    ...
    /* Stopping diagnostic and HealthStatsManager services,
     * To avoid race condition, between installer app, diagnostic and HealthStatsManager services
     */

    if(!stop_service("diagnostic.service")) {
        sleep(UNIT_TIME);
        stop_service("diagnostic.service");
    }

    if(!stop_service("HealthStatsManager.service")) {
        sleep(UNIT_TIME);
        stop_service("HealthStatsManager.service");
    }
...
bool queue_server_creation(){
   ...
   msg_reader_q = nd_msgq_t::get_msgq( "q_ins_read", nd_msgq_t::ND_MSGQ_SERVER);
   ...
   msg_response_q = nd_msgq_t::get_msgq( "q_ins_resp", nd_msgq_t::ND_MSGQ_SERVER);
...
bool response_mdvr_wifi_strength(int sequence) {
    ...
    if(ext_cam_feature_enabled) { 
        string cmd = "iw wlan0 scan | grep -B10 " + mdvr_ssid_global + " | grep \"signal\" | cut -d ':' -f2 | cut -d ' ' -f2 | cut -d '.' -f1";
        cmd_resp = "";
        LOG_I(TAG, "strength cmd: %s", cmd.c_str());
        if(system_execute_with_resp("MDVR_WIFI_STRENGTH", cmd, cmd_resp)) {
            if(string_to_integer(cmd_resp, wifi_strength)) {
                response = true;
                LOG_I(TAG, "wifi strength: %d dBm", wifi_strength);
                wifi_quality = wifi_strength;
            }
        }
    }
...
bool handle_pair_unpair_accessory(char *pair_unpair_data, string &error_code_str, bool pair_accessory) {
    ...
    do
    {
        ...
        for (size_t i = 0; i < data_array_size; i++) {
            string accessory_id = "not_defined";
            int err = 1;
            do {
                ...
                while (retry_count < MAX_RETRY_COUNT) {
                    db_update_status = nd::device::update_accessories_db(accessory_id, accessory_type, data_string,
                                                                         pair_accessory, db_type);
                    if (db_update_status) {
        ...
        LOG_I(TAG, "error code json: %s", json_dumps(error_code_json, JSON_COMPACT));
...
static bool execute_aplay_command(const string &aplay_cmd, string &result) {
    bool ret = false;
    
    do {
        // Extract file path from aplay command
        size_t aplay_pos = aplay_cmd.find("aplay");
        
        if(aplay_pos == string::npos) {
            LOG_E(TAG, "aplay keyword not found in command");
            break;
        }
        
        // Get substring after "aplay "
        string filepath = aplay_cmd.substr(aplay_pos + 6); // 6 = length of "aplay "
        
        // Trim leading and trailing whitespace
        filepath = trim(filepath);
        
        if(filepath.empty()) {
            LOG_E(TAG, "Extracted audio file path is empty");
            break;
        }
        
        LOG_I(TAG, "Extracted audio file path: %s", filepath.c_str());
        
        // Use start_stream_playback() API with the file path
        start_stream_playback(filepath.c_str());
        
        result = "Stream playback completed successfully";
        ret = true;
        
    } while(false);
    
    if(!ret) {
        LOG_E(TAG, "execute_aplay_command failed");
        result = "Stream playback failed";
    }
...
bool cmd_execute_with_timeout(void*args){
    ...
    // Check if generic command contains "aplay" substring to handle audio playback using API instead of 'aplay' system call.
    if(generic_str.str().find("aplay") != string::npos) {
        LOG_I(TAG, "aplay command found in generic command. Executing via start_stream_playback() API");
        
        ret = execute_aplay_command(generic_str.str(), generic_cmd_result);
        
        if(!ret) {
            LOG_E(TAG, "failed to execute aplay command");
            resp.response = false;
        } else {
            resp.response = true;
        }
    } else {
        // Execute non-aplay commands normally
        LOG_I(TAG, "Executing generic command: %s", generic_str.str().c_str());
...
bool response_generic_cmd(int sequence, int length, bool gen_cmd_flag) {
    ...
    if (gen_cmd_flag){
       ...
       LOG_I(TAG, "command read from file : %s", generic_str.str());
            ...
            case CREATE_INSTALLER_SOCKET:
                {
                    if(listenfd == -1) {
                        // Restart obd binary service hotspot is activated
                        if(pthread_create(&obd_run_thread, NULL, obd_restart_thread_func, NULL) != 0) {
                            LOG_E(TAG, "Failed to create obd restart thread");
                        }
...
int main (int argc, char **argv)
{
  ...
  nd_device_obj_init();
   ...
   LOG_I(TAG, "value of socket des from create socket fun %d\n", connfd);
    ...
    // TODO: Handle
    // // send stop led blink msg to led nd_central service
    // result = stop_led_blinking();
    // if (!result){
    //     LOG_E(TAG, "not able to stop led blinking but continue\n");
    // }
