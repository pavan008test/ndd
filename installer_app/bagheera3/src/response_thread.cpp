...
bool testconn_response_to_app(){
   ...
   if(device_type == "krait") {
       json_object_set_new( json_1, "stream_port", json_string("8900"));
   } else {
       json_object_set_new( json_1, "stream_port", json_string("4000"));
   }
...
bool validate_dms_response_to_app() {
    bool ret = false;
    int sequence_response;
    bool response = false;
    int error_code;
    ...
    do {
        response_validate_dms_msg_t *g_msg = (response_validate_dms_msg_t *)msg_response->get_buffer();

        sequence_response = g_msg->sequence_no;
        response = g_msg->response;
        error_code = g_msg->err_code;

        json_t *json_1 =json_object();

        json_object_set_new( json_1, "cmd", json_string("res_validate_dms"));
        json_object_set_new( json_1, "error", json_integer(error_code));
        json_object_set_new( json_1, "seq", json_integer(sequence_response));
        if (response) {
            json_object_set_new( json_1, "res_status", json_boolean(1));
        }
        ...
        char *res_json_char = json_dumps(json_1,0);

        if(res_json_char == NULL){
            LOG_E(TAG,"JSON creation failed for validate dms response\n");
            break;
        }
        ...
        // push the json to app
        write_socket(res_json_char, strlen(res_json_char));
...
bool end_dms_session_response_to_app() {
    bool ret = false;
    int sequence_response;
    bool response = false;
    json_t *json_1;
    do {
        response_end_dms_session_msg_t *g_msg = (response_end_dms_session_msg_t *)msg_response->get_buffer();
        ...
        json_1 =json_object();
        if (json_1 == NULL) {
            LOG_E(TAG,"JSON creation failed for end dms session response\n");
            break;
        }

        json_object_set_new( json_1, "cmd", json_string("res_end_dms_session"));
        json_object_set_new( json_1, "seq", json_integer(sequence_response));
        if (response) {
            json_object_set_new( json_1, "res_status", json_boolean(1));
        }
...
bool dms_calibration_response_to_app(string encoded_img) {
    bool ret = false;
    int sequence_response;
    bool response = false;
    json_t *json_1;
    do {
        response_dms_calibration_msg_t *g_msg = (response_dms_calibration_msg_t *)msg_response->get_buffer();
        ...
        json_object_set_new( json_1, "cmd", json_string("res_dms_calibration"));
        json_object_set_new( json_1, "seq", json_integer(sequence_response));
        if (response) {
            json_object_set_new( json_1, "res_status", json_boolean(1));
        }
        ...
        json_object_set_new( json_1, "pitch_correction", json_integer(g_msg->pitch_correction));
        json_object_set_new( json_1, "yaw_correction", json_integer(g_msg->yaw_correction));
        json_object_set_new( json_1, "roll_correction", json_integer(g_msg->roll_correction));
        json_object_set_new( json_1, "cam_distance_correction", json_integer(g_msg->cam_distance_correction));
        json_object_set_new( json_1, "calib_status_code", json_integer(g_msg->calib_status_code));
        json_object_set_new( json_1, "img", json_string(encoded_img.c_str()));

        char *res_json_char = json_dumps(json_1,0);

        if(res_json_char == NULL){
            LOG_E(TAG,"JSON creation failed for dms calibration response\n");
            break;
        }
...
bool toggle_annotation_response_to_app() {
    bool ret = false;
    int sequence_response;
    bool response = false;
    json_t *json_1;
    do {
        response_toggle_annotation_msg_t *g_msg = (response_toggle_annotation_msg_t *)msg_response->get_buffer();
        ...
        json_object_set_new( json_1, "cmd", json_string("res_toggle_annotation"));
        json_object_set_new( json_1, "seq", json_integer(sequence_response));
        if (response) {
            json_object_set_new( json_1, "res_status", json_boolean(1));
        }
        ...
        if(res_json_char == NULL){
            LOG_E(TAG,"JSON creation failed for toggle annotation response\n");
            break;
        }
...
bool rotate_stream_response_to_app() {
    bool ret = false;
    int sequence_response;
    bool response = false;
    int error_code = -1;
    json_t *json_1 = nullptr;
    do {
        response_rotate_stream_msg_t *g_msg = (response_rotate_stream_msg_t *)msg_response->get_buffer();

        sequence_response = g_msg->sequence_no;
        response = g_msg->response;
        error_code = g_msg->error_code;

        json_1 =json_object();
        if (json_1 == NULL) {
            LOG_E(TAG,"JSON creation failed for rotate stream response\n");
            break;
        }
        json_object_set_new( json_1, "cmd", json_string("res_rotate_cam"));
        json_object_set_new( json_1, "seq", json_integer(sequence_response));
        if (response) {
            json_object_set_new( json_1, "res_status", json_boolean(1));
        }
        ...
        json_object_set_new(json_1, "error_code", json_integer(error_code));

        char *res_json_char = json_dumps(json_1,0);

        if(res_json_char == NULL){
            LOG_E(TAG,"JSON creation failed for flip stream response\n");
            break;
        }
...
bool get_encoded_image(string &encoded_image) {
    ifstream nameFileout;
    stringstream ss;

    nameFileout.open(base64_encoded_calibration_image_file);
    getline(nameFileout, encoded_image);
    nameFileout.close();

    /* delete the json file */
    if (!file_delete(base64_encoded_calibration_image_file))
        LOG_E(TAG, "file delete failed for file: %s", base64_encoded_calibration_image_file.c_str());
 ...
 while (1){
   ...
   switch (g_msg->msg_type){
       ...
       case RESPONSE_FETCH_ALL_ACCESSORY:
        ...
        case RESPONSE_VALIDATE_DMS:
        {
            validate_dms_response_to_app();
            break;
        }
       case RESPONSE_DMS_CALIBRATION:
        {
            string encoded_image;
            bool ret = get_encoded_image(encoded_image);
            if(ret == false) {
                LOG_E(TAG, "encoded image not found");
            }

            dms_calibration_response_to_app(encoded_image);
            break;
        }
       case RESPONSE_END_DMS_SESSION:
        {
            end_dms_session_response_to_app();
            break;
        }
       case RESPONSE_TOGGLE_ANNOTATION:
        {
            toggle_annotation_response_to_app();
            break;
        }
       case RESPONSE_ROTATE_STREAM:
       {
            rotate_stream_response_to_app();
            break;
       }
