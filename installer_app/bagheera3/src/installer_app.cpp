...
#include "dms_analytics_interface.hpp"
#include "Base64.cpp"
#include <curl/curl.h>
...
/* BSP parameters for handling USB Connect/Disconnect
 * and getting SN for DMS camera */
static dmsCam_t dmsCam;
static unsigned char dmsCam_SN[SN_LEN + 1];
static unsigned char dmsCam_PN[PN_LEN + 1];

/* Analytics Library Handle for annotation */
static void* dms_analytics_handle = NULL;
static int frame_count = 0;

/* DMS camera streaming pipeline related */
static GstElement *elem_appsrc_analytics = NULL;
static GstElement *pipeline_appsrc = NULL;
static stringstream gst_appsrc_pipeline_str;
static int dms_analytics_width = 640;
static int dms_analytics_height = 480;
static int dms_camera_width = 1600;
static int dms_camera_height = 1200;
static int dms_camera_fps = 30;
static int dms_camera_bitrate = 8000000;

/* DMS camera calibration recording pipeline related */
static stringstream gst_dms_calibration_video_pipeline_str;
static GstElement *pipeline_dms_calibration_video = NULL;
static GstElement *elem_appsrc_analytics_calibration_video = NULL;
static bool is_calibration_started = false;
static const string base_file_path = "/home/ubuntu/.nddevice/";
static const string base_file_name = "dms_calibration_";
static const string dms_calibration_recording_file = "/home/ubuntu/.nddevice/dms_calibration_video.mp4";
static const string base_folder_dms_calibration_data = "/home/ubuntu/.nddevice/dms_calibration_data/";
static string base_upload_url_dms_calibration_data = "";

/* DMS camera final calibration image pipeline related */
static stringstream gst_dms_calibration_image_pipeline_str;
static GstElement *pipeline_dms_calibration_image = NULL;
static GstElement *elem_appsrc_analytics_calibration_image = NULL;

/* Calibration End Signal related */
static pthread_mutex_t analytics_calibration_mutex = PTHREAD_MUTEX_INITIALIZER;
static pthread_cond_t analytics_calibration_end_cond;
static app_an::DMSCalibrationStatus final_dms_calibration_data;

/* Final DMS calibration image with base64 encoded ready signal */
static pthread_mutex_t analytics_calibration_image_mutex = PTHREAD_MUTEX_INITIALIZER;
static pthread_cond_t analytics_calibration_base64_cond;
static int dms_calibrated_image_width = 640;
static int dms_calibrated_image_height = 480;

/* For DMS annotation enable/disable */
static int is_annotation_enabled = 1;

NDService *nd_service_obj; //nd service object, to detect crashes
/* If DMS camera is mounted opposite, flipping will be required */
static int dms_camera_flip = 0;
static string CIRCULAR_BUFFER_PATH = "";

/* To test streaming without analytics, for debugging purpose */
static bool dms_skip_analytics = false;
...
bool read_configurations(){
    ...
    if (!bagheera_config_parser.getParseStatus()){
        ...
        string_to_integer(bagheera_config_parser.getConfig("INSTALLER_APP","dms_camera_flip", "0", get_override_val, is_val_overridden), dms_camera_flip);
        dms_skip_analytics = (bagheera_config_parser.getConfig("INSTALLER_APP","dms_skip_analytics", "false", get_override_val, is_val_overridden) == "true");
    }
    LOG_I(TAG, "following configuration from read_configuration function\n");
    LOG_I(TAG, "ip_address:%s, port_address:%s, socket_timeout: %s, wlan_iface: %s, dms_camera_flip:%d, dms_skip_analytics:%d",
              ip_address.c_str(), port_address.c_str(), socket_timeout.c_str(), wlan_interface.c_str(), dms_camera_flip, dms_skip_analytics);
    ...
    /* upload URL is updated based on staging or prod */
    string cloud_config_path = "/home/ubuntu/.nddevice/latest/cloudconfig.ini";
    Config_parser cloud_config_parser (cloud_config_path);
    string cloud_server = cloud_config_parser.getConfig("cloud","server","prod");
    string server_url = cloud_config_parser.getConfig(cloud_server,"ota","");
    base_upload_url_dms_calibration_data = server_url + "/v1/upload/calibration/video?calibrationUuid=";
    LOG_I(TAG, "base_upload_url_dms_calibration_data: %s", base_upload_url_dms_calibration_data.c_str());
...
/* Callback from analytics, at the end of calibration */
static bool on_calibration_end_cb_func(app_an::DMSCalibrationStatus calibration_status) {
    LOG_I(TAG, "call for on_calibration_end_cb_func: calibration status: %d, image pointer is %x", calibration_status.status, calibration_status.calib_image_data_nv12_ptr);

    final_dms_calibration_data = calibration_status;

    pthread_mutex_lock(&analytics_calibration_mutex);
    pthread_cond_signal(&analytics_calibration_end_cond);
    pthread_mutex_unlock(&analytics_calibration_mutex);
    return true;
}

/*Description:
 *This is the callback registered with appsink which receive a single jpeg image
 *and encode it to base64
 */
static void appsink_analytics_jpeg_cb(GstAppSink *object, gpointer user_data)
{
    GstFlowReturn ret;
    GstCaps* caps = NULL;
    GstStructure *s = NULL;
    ...
    LOG_D(TAG, "came inside appsink_analytics_jpeg_cb");
    ...
    string res_str = encode64(string((char*)map.data, map.size));

    FILE* fptr = NULL;
    fptr = fopen(base64_encoded_calibration_image_file.c_str(), "w");
    if (fptr == NULL) {
        LOG_E(TAG, "base64_encoded_calibration_image_file not present");
    } else {
        int write_bytes = fwrite(res_str.c_str(), sizeof(char), res_str.length(), fptr);
        LOG_I(TAG, "written %d bytes to base64 encoded file", write_bytes);
        fclose(fptr);
    }
    ...
    pthread_mutex_lock(&analytics_calibration_image_mutex);
    pthread_cond_signal(&analytics_calibration_base64_cond);
    pthread_mutex_unlock(&analytics_calibration_image_mutex);

    return;
}

/*Description:
 *This is the callback registered with appsink which feeds raw frames to analytics for
 *processing at 30 fps
 */
static void appsink_analytics_cb(GstAppSink *object, gpointer user_data)
{
    GstFlowReturn ret;
    ...
    if (!dms_skip_analytics) {
        GstBuffer *dst_buffer = gst_buffer_copy_deep(buffer);

        GstMapInfo map_dst;
        gst_buffer_map (dst_buffer, &map_dst, GST_MAP_WRITE);

        app_an::DMSCalibrationStatus calibration_status = app_an::process_and_annotate(dms_analytics_handle, frame_count, map.data, map_dst.data);
        LOG_D(TAG, "calibration summary: status:%d, progress:%d, yaw:%f, pitch:%f, roll:%f",
                   calibration_status.status, calibration_status.progress, calibration_status.yaw, calibration_status.pitch, calibration_status.roll);

        frame_count++;

        if (is_calibration_started) {
            gst_buffer_ref(dst_buffer);
            ret = gst_app_src_push_buffer((GstAppSrc*)elem_appsrc_analytics_calibration_video, dst_buffer);
            LOG_E(TAG, "elem_appsrc_analytics_calibration_video pushed buffer returned with %d", ret);
        }

        if (is_annotation_enabled) {
            ret = gst_app_src_push_buffer((GstAppSrc*)elem_appsrc_analytics, dst_buffer);
            if (map_dst.data != nullptr) {
                gst_buffer_unmap(dst_buffer, &map_dst);
            }
            if (ret != GST_FLOW_OK) {
                LOG_E(TAG, "gst_app_src_push_buffer failed with %d", ret);
                if (dst_buffer != nullptr) {
                    gst_buffer_unref(dst_buffer);
                }
            }
        } else {
            GstBuffer *buffer_cpy = gst_buffer_copy_deep(buffer);
            ret = gst_app_src_push_buffer((GstAppSrc*)elem_appsrc_analytics, buffer_cpy);
            if (ret != GST_FLOW_OK) {
                LOG_E(TAG, "gst_app_src_push_buffer failed with %d", ret);
                if (buffer_cpy != nullptr) {
                    gst_buffer_unref(buffer_cpy);
                }
            }
            if (map_dst.data != nullptr) {
                gst_buffer_unmap(dst_buffer, &map_dst);
...
bool start_streaming(int argument, int sequence_no){
    ...
    gst_pipeline_str.str(std::string());
    gst_appsrc_pipeline_str.str(std::string());
    gst_dms_calibration_video_pipeline_str.str(std::string());
    gst_dms_calibration_image_pipeline_str.str(std::string());

    LOG_I(TAG, "******camera string is %s ********\n", camera_no_string);

    if (argument < BACK_CAM || argument > DMS_CAM) {
	    ...
	    if( eBagheera_2 == nd_device_obj->getDeviceType() )  {
		   gst_pipeline_str
			    ...
			    << "rtph265pay pt=96 ! udpsink host=" << client_ip_address << " port=4000";
	    } else if (eBagheera_3 == nd_device_obj->getDeviceType()) {
		   gst_pipeline_str
			    << "nvv4l2camerasrc device=/dev/video0 ! "
                << "video/x-raw(memory:NVMM), format=UYVY, width=1920, height=1080 ! "
			    << "nvvidconv ! video/x-raw(memory:NVMM), format=NV12, width=640, height=352! "
			    // idrinterval=15  will reduce the streaming latency
			    << "nvv4l2h265enc name=h265enc control-rate=0 bitrate=300000  peak-bitrate=300000 insert-sps-pps=true iframeinterval=30 idrinterval=30 ! "
			    << "h265parse ! video/x-h265,stream-format=(string)byte-stream ! "
			    << "rtph265pay pt=96 ! udpsink host=" << client_ip_address << " port=4000 sync=FALSE";
	    }
    } else if (argument >= EXT_CAM_1 && argument <= EXT_CAM_4) {
        ...
        usleep(HUNDRED_MS);
        return true;
    } else if (argument == DMS_CAM) {
        if (isDMSsupported(nd_device_obj->getDeviceType())) {
            dms_analytics_handle = app_an::setup(dms_analytics_width, dms_analytics_height);
            LOG_I(TAG, "dms_analytics_handle created %x", dms_analytics_handle);
            if (dms_analytics_handle == NULL) {
                LOG_E(TAG, "app_an::setup has failed");
                return response_streaming(argument, sequence_no, false);
            }
            app_an::register_calibration_end_cb(dms_analytics_handle, on_calibration_end_cb_func);

            gst_pipeline_str
                << "v4l2src name=dms_camera device=/dev/dms_h264 ! "
                << "video/x-h264, stream-format=(string)byte-stream ! nvv4l2decoder ! "
                << "video/x-raw(memory:NVMM), format=NV12, width=" << dms_camera_width << ", height="<< dms_camera_height << " ! "
                << "nvvidconv output-buffers=20 ! video/x-raw, format=NV12, width=" << dms_analytics_width << ", height="<< dms_analytics_height << " ! "
                << "appsink name=appsink_analytics emit-signals=TRUE sync=FALSE";

            gst_appsrc_pipeline_str
                << "appsrc name=appsrc_analytics ! "
                << "queue ! video/x-raw, format=NV12, width=" << dms_analytics_width << ", height="<< dms_analytics_height << " ! "
                << "nvvidconv output-buffers=20 ! video/x-raw(memory:NVMM), format=NV12, width=" << dms_analytics_width << ", height="<< dms_analytics_height << " ! "
                << "nvv4l2h265enc name=h265enc control-rate=0 bitrate=800000  peak-bitrate=800000 insert-sps-pps=true iframeinterval=30 idrinterval=30 ! "
                << "h265parse ! video/x-h265,stream-format=(string)byte-stream ! "
                << "rtph265pay pt=96 ! udpsink name=udpsink_dms host=" << client_ip_address << " port=4000 sync=FALSE";

            gst_dms_calibration_video_pipeline_str
                << "appsrc name=appsrc_analytics_calibration_video ! "
                << "queue ! video/x-raw, format=NV12, width=" << dms_analytics_width << ", height="<< dms_analytics_height << " ! "
                << "nvvidconv name=dms_video_conv output-buffers=20 ! video/x-raw(memory:NVMM), format=NV12, width=" << dms_analytics_width << ", height="<< dms_analytics_height << " ! "
				<< "nvv4l2h265enc name=dms_video_enc control-rate=0 bitrate=500000 peak-bitrate=500000 preset-level=3 maxperf-enable=1 idrinterval=0 iframeinterval=30 ! "
                << "h265parse name=dms_video_parse ! matroskamux ! filesink location=" << dms_calibration_recording_file.c_str();

            gst_dms_calibration_image_pipeline_str
                << "appsrc name=appsrc_analytics_calibration_image ! "
                << "queue ! video/x-raw, format=NV12, width=" << dms_analytics_width << ", height="<< dms_analytics_height << ", framerate=1/1 ! "
                << "nvvidconv ! video/x-raw(memory:NVMM), format=NV12, width=" << dms_calibrated_image_width << ", height="<< dms_calibrated_image_height << " ! "
                << "nvjpegenc ! "
                << "appsink name=appsink_analytics_image emit-signals=TRUE sync=FALSE";

            GError* err = NULL;
            LOG_I(TAG, "command for appsrc_analytics pipeline\n");
            string temp_str1 = gst_appsrc_pipeline_str.str();
            printf("%s\n", temp_str1.c_str());
            pipeline_appsrc = gst_parse_launch ((temp_str1.c_str()), &err);

            if (err != NULL) {
                LOG_E(TAG, "gstreamer failed to create appsrc pipeline\n");
                LOG_E(TAG, "%s\n", err->message);
                g_error_free(err);
                return response_streaming(argument, sequence_no, false);
            }

            if (pipeline_appsrc == NULL) {
                LOG_E(TAG, "gst parse launch failed\n");
                gst_object_unref(pipeline_appsrc);
                //send error message to app
                return response_streaming(argument, sequence_no, false);
            }
            elem_appsrc_analytics = gst_bin_get_by_name(GST_BIN(pipeline_appsrc), "appsrc_analytics");
            g_object_set (G_OBJECT (elem_appsrc_analytics), "caps",
                                    gst_caps_new_simple ("video/x-raw",
                                    "format", G_TYPE_STRING, "NV12",
                                    "width", G_TYPE_INT, dms_analytics_width,
                                    "height", G_TYPE_INT, dms_analytics_height,
                                    "framerate", GST_TYPE_FRACTION, 30, 1,
                                    NULL), NULL);

            LOG_I(TAG, "command for appsrc_analytics_calibration_video pipeline\n");
            string temp_str2 = gst_dms_calibration_video_pipeline_str.str();
            printf("%s\n", temp_str2.c_str());
            pipeline_dms_calibration_video = gst_parse_launch ((temp_str2.c_str()), &err);

            if (err != NULL) {
                LOG_E(TAG, "gstreamer failed to create dms calibration video pipeline\n");
                LOG_E(TAG, "%s\n", err->message);
                g_error_free(err);
                return response_streaming(argument, sequence_no, false);
            }

            if (pipeline_dms_calibration_video == NULL) {
                LOG_E(TAG, "gst parse launch failed\n");
                gst_object_unref(pipeline_dms_calibration_video);
                //send error message to app
                return response_streaming(argument, sequence_no, false);
            }
            elem_appsrc_analytics_calibration_video = gst_bin_get_by_name(GST_BIN(pipeline_dms_calibration_video), "appsrc_analytics_calibration_video");
            g_object_set (G_OBJECT (elem_appsrc_analytics_calibration_video), "caps",
                                    gst_caps_new_simple ("video/x-raw",
                                    "format", G_TYPE_STRING, "NV12",
                                    "width", G_TYPE_INT, dms_analytics_width,
                                    "height", G_TYPE_INT, dms_analytics_height,
            ...
            LOG_I(TAG, "command for appsrc_analytics_calibration_image pipeline\n");
            string temp_str3 = gst_dms_calibration_image_pipeline_str.str();
            printf("%s\n", temp_str3.c_str());
            pipeline_dms_calibration_image = gst_parse_launch ((temp_str3.c_str()), &err);

            if (err != NULL) {
                LOG_E(TAG, "gstreamer failed to create dms calibration image pipeline\n");
                LOG_E(TAG, "%s\n", err->message);
                g_error_free(err);
                return response_streaming(argument, sequence_no, false);
            }

            if (pipeline_dms_calibration_image == NULL) {
                LOG_E(TAG, "gst parse launch failed\n");
                gst_object_unref(pipeline_dms_calibration_image);
                //send error message to app
                return response_streaming(argument, sequence_no, false);
            }
            elem_appsrc_analytics_calibration_image = gst_bin_get_by_name(GST_BIN(pipeline_dms_calibration_image), "appsrc_analytics_calibration_image");
            g_object_set (G_OBJECT (elem_appsrc_analytics_calibration_image), "caps",
                                    gst_caps_new_simple ("video/x-raw",
                                    "format", G_TYPE_STRING, "NV12",
                                    "width", G_TYPE_INT, dms_analytics_width,
                                    "height", G_TYPE_INT, dms_analytics_height,
            ...
            GstElement *elem_analytics_image = gst_bin_get_by_name(GST_BIN(pipeline_dms_calibration_image), "appsink_analytics_image");
            if (elem_analytics_image) {
                g_signal_connect(elem_analytics_image, "new-sample", G_CALLBACK (appsink_analytics_jpeg_cb), pipeline);
                g_object_unref (elem_analytics_image);
            }
        }
    } else {
        ...
        if( eBagheera_2 == nd_device_obj->getDeviceType() )  {
            gst_pipeline_str
                ...
                << "udpsink host=" << client_ip_address <<  " port=4000 sync=FALSE";
        } else if (eBagheera_3 == nd_device_obj->getDeviceType()) {

            int sensor_mode = 1; //1600x900 15fps
            if (argument == BACK_CAM ) {
                    sensor_mode = 0; //1920x1080 15fps
            }
            gst_pipeline_str
                << "nvarguscamerasrc sensor-id=" << camera_no_string << " sensor-mode=" << sensor_mode
                << " ! video/x-raw(memory:NVMM),width=1920, height=1080, framerate=30/1, format=NV12 ! "
			    << "nvvidconv ! video/x-raw(memory:NVMM), format=NV12, width=640, height=352! "
                << "nvv4l2h265enc control-rate=0 bitrate=1000000 peak-bitrate=1000000 insert-sps-pps=true "
                << "iframeinterval=30 idrinterval=15 ! "
                << "h265parse ! queue ! "
                << "video/x-h265,stream-format=(string)byte-stream ! rtph265pay pt=96 ! "
                << "udpsink host=" << client_ip_address <<  " port=4000 sync=FALSE";
        }
    ...
    printf("%s\n", gst_pipeline_str.str().c_str());

    GError* err = NULL;
    pipeline = gst_parse_launch ((gst_pipeline_str.str().c_str()), &err);
    if( err != NULL ) {
    ...
    GstElement *elem_analytics = gst_bin_get_by_name(GST_BIN(pipeline), "appsink_analytics");
    if (elem_analytics) {
        g_signal_connect(elem_analytics, "new-sample", G_CALLBACK (appsink_analytics_cb), pipeline);
        g_object_unref (elem_analytics);
    }
    ...
    if (pipeline_appsrc) {
        ret = gst_element_set_state(pipeline_appsrc, GST_STATE_PLAYING);
        if (ret == GST_STATE_CHANGE_FAILURE) {
            LOG_E(TAG, "Unable to set the pipeline_appsrc to the playing state.\n");
            gst_object_unref(pipeline_appsrc);
            pipeline_appsrc = NULL;
            return response_streaming(argument, sequence_no, false);
        } else {
            LOG_I(TAG, "gst_element_set_state: %d\n", ret);
        }
    ...
    if (pipeline_dms_calibration_video) {
        ret = gst_element_set_state(pipeline_dms_calibration_video, GST_STATE_PLAYING);
        if (ret == GST_STATE_CHANGE_FAILURE) {
            LOG_E(TAG, "Unable to set the pipeline_dms_calibration_video to the playing state.\n");
            gst_object_unref(pipeline_dms_calibration_video);
            pipeline_dms_calibration_video = NULL;
            return response_streaming(argument, sequence_no, false);
        } else {
            LOG_I(TAG, "gst_element_set_state: %d\n", ret);
        }
    ...
    if (pipeline_dms_calibration_image) {
        ret = gst_element_set_state(pipeline_dms_calibration_image, GST_STATE_PLAYING);
        if (ret == GST_STATE_CHANGE_FAILURE) {
            LOG_E(TAG, "Unable to set the pipeline_dms_calibration_image to the playing state.\n");
            gst_object_unref(pipeline_dms_calibration_image);
            pipeline_dms_calibration_image = NULL;
            return response_streaming(argument, sequence_no, false);
        } else {
            LOG_I(TAG, "gst_element_set_state: %d\n", ret);
        }
...
bool clear_pipeline(int cam_num) {
    ...
    if (NULL != pipeline_appsrc) {
        gboolean eventStatus = gst_element_send_event (pipeline_appsrc, gst_event_new_eos());
        LOG_I(TAG, "gst_element_send_event eventStatus: %d", eventStatus);

        GstStateChangeReturn retValue = gst_element_set_state(pipeline_appsrc, GST_STATE_PAUSED);
        LOG_I(TAG, "gst_element_set_state retValue: %d", retValue);

        retValue = gst_element_get_state(pipeline_appsrc, &state, &pending, GST_CLOCK_TIME_NONE);
        LOG_I(TAG, "gst_element_get_state state: %d, pending: %d, retValue: %d", state, pending, retValue);
        ...
        gst_element_set_state(pipeline_appsrc, GST_STATE_READY);
        retValue = gst_element_get_state(pipeline_appsrc, &state, &pending, GST_CLOCK_TIME_NONE);
        ...
        gst_element_set_state(pipeline_appsrc, GST_STATE_NULL);
        LOG_I(TAG, "pipeline_appsrc state changed to NULL");

        usleep(100*1000);
        LOG_I(TAG, "gst_element_get_state state: %d, pending: %d, retValue: %d", state, pending, retValue);

        gst_object_unref(GST_OBJECT(pipeline_appsrc));
        pipeline_appsrc = NULL;
    } else if (cam_num == DMS_CAM) {
        gst_appsrc_pipeline_str.str("");
        LOG_E(TAG, "pipeline_appsrc NULL here: %s:%d\n", __func__, __LINE__);
        return false;
    }

    if (NULL != pipeline_dms_calibration_video) {
        GstState pending, state;
        gboolean eventStatus = gst_element_send_event (pipeline_dms_calibration_video, gst_event_new_eos());
        LOG_I(TAG, "gst_element_send_event eventStatus: %d", eventStatus);

        GstStateChangeReturn retValue = gst_element_set_state(pipeline_dms_calibration_video, GST_STATE_PAUSED);
        LOG_I(TAG, "gst_element_set_state retValue: %d", retValue);

        retValue = gst_element_get_state(pipeline_dms_calibration_video, &state, &pending, GST_CLOCK_TIME_NONE);
        LOG_I(TAG, "gst_element_get_state state: %d, pending: %d, retValue: %d", state, pending, retValue);
        ...
        gst_element_set_state(pipeline_dms_calibration_video, GST_STATE_READY);
        retValue = gst_element_get_state(pipeline_dms_calibration_video, &state, &pending, GST_CLOCK_TIME_NONE);

        if ((GST_STATE_CHANGE_SUCCESS == retValue) && (GST_STATE_READY == state)) {
            LOG_I(TAG, "success and Ready");
        }
        gst_element_set_state(pipeline_dms_calibration_video, GST_STATE_NULL);
        LOG_I(TAG, "pipeline_dms_calibration_video state changed to NULL");

        usleep(100*1000);
        LOG_I(TAG, "gst_element_get_state state: %d, pending: %d, retValue: %d", state, pending, retValue);

        gst_object_unref(GST_OBJECT(pipeline_dms_calibration_video));
        pipeline_dms_calibration_video = NULL;
    } else if (cam_num == DMS_CAM) {
        gst_dms_calibration_video_pipeline_str.str("");
        LOG_E(TAG, "pipeline_dms_calibration_video NULL here: %s:%d\n", __func__, __LINE__);
        return false;
    }

    if (NULL != pipeline_dms_calibration_image) {
        GstState pending, state;
        gboolean eventStatus = gst_element_send_event (pipeline_dms_calibration_image, gst_event_new_eos());
        LOG_I(TAG, "gst_element_send_event eventStatus: %d", eventStatus);

        GstStateChangeReturn retValue = gst_element_set_state(pipeline_dms_calibration_image, GST_STATE_PAUSED);
        LOG_I(TAG, "gst_element_set_state retValue: %d", retValue);

        retValue = gst_element_get_state(pipeline_dms_calibration_image, &state, &pending, GST_CLOCK_TIME_NONE);
        LOG_I(TAG, "gst_element_get_state state: %d, pending: %d, retValue: %d", state, pending, retValue);
        ...
        gst_element_set_state(pipeline_dms_calibration_image, GST_STATE_READY);
        retValue = gst_element_get_state(pipeline_dms_calibration_image, &state, &pending, GST_CLOCK_TIME_NONE);

        if ((GST_STATE_CHANGE_SUCCESS == retValue) && (GST_STATE_READY == state)) {
            LOG_I(TAG, "success and Ready");
        }
        gst_element_set_state(pipeline_dms_calibration_image, GST_STATE_NULL);
        LOG_I(TAG, "pipeline_dms_calibration_image state changed to NULL");

        usleep(100*1000);
        LOG_I(TAG, "gst_element_get_state state: %d, pending: %d, retValue: %d", state, pending, retValue);

        gst_object_unref(GST_OBJECT(pipeline_dms_calibration_image));
        pipeline_dms_calibration_image = NULL;
    } else if (cam_num == DMS_CAM) {
        gst_dms_calibration_image_pipeline_str.str("");
        LOG_E(TAG, "pipeline_dms_calibration_image NULL here: %s:%d\n", __func__, __LINE__);
        return false;
    }
...
bool stop_streaming(int cam_num){
        ...
        if (cam_num == currently_streaming){
           status = clear_pipeline(cam_num);
           if (currently_streaming >= EXT_CAM_1 && currently_streaming <= EXT_CAM_4) {
               pthread_mutex_lock(&ext_cam_data_mutex);
               curently_stream_ext_file_idx[currently_streaming-3] = -1;
               pthread_mutex_unlock(&ext_cam_data_mutex);
               close(transfd);
           ...
           if (cam_num == DMS_CAM) {
               app_an::destroy(dms_analytics_handle);
               dms_analytics_handle = NULL;
               //dmsUnInit(&dmsCam);
           }
           return status;
        }
        else {
            if (currently_streaming == -1) {
                LOG_I(TAG, "Already cleared the pipeline");
                return true;
            }
...
int gracefull_exit(int connfd) {
    if (NULL != pipeline) {
        ...
        LOG_I(TAG, "gst_element_set_state ret: %d\n", retValue);
    ...
    if (NULL != pipeline_appsrc) {
        ...
        auto retValue = gst_element_set_state(pipeline_appsrc, GST_STATE_PAUSED);
        LOG_I(TAG, "gst_element_set_state ret: %d", retValue);
        ...
        retValue = gst_element_set_state(pipeline_appsrc, GST_STATE_NULL);
        LOG_I(TAG, "gst_element_set_state ret: %d\n", retValue);
    ...
    if (NULL != pipeline_dms_calibration_video) {
        ...
        auto retValue = gst_element_set_state(pipeline_dms_calibration_video, GST_STATE_PAUSED);
        LOG_I(TAG, "gst_element_set_state ret: %d", retValue);
        ...
        retValue = gst_element_set_state(pipeline_dms_calibration_video, GST_STATE_NULL);
        LOG_I(TAG, "gst_element_set_state ret: %d\n", retValue);
    ...
    if (NULL != pipeline_dms_calibration_image) {
        ...
        auto retValue = gst_element_set_state(pipeline_dms_calibration_image, GST_STATE_PAUSED);
        LOG_I(TAG, "gst_element_set_state ret: %d", retValue);
        ...
        retValue = gst_element_set_state(pipeline_dms_calibration_image, GST_STATE_NULL);
        LOG_I(TAG, "gst_element_set_state ret: %d\n", retValue);
...
static const string stop_all_services = "systemctl stop `find /home/ubuntu/.nddevice/latest/service/ -name *.service ! -name nd_shutdown.service ! -name nvcamera-daemon.service -printf %f`";

     ret = system_execute("stopping all services", stop_all_services);
     if (ret ==0)
        return true;
     else
        return false;
...
bool stop_transcoder_service(){

    static const string stop_transcoder = "systemctl stop transcoder.service";

    int ret =0;

    ret = system_execute ("stopping transcoder service", stop_transcoder);

    if (ret !=0){
       LOG_E(TAG, "transcoder service is not stopped after client connected\n");
       return false;
    }
...
bool response_validate_dms(int sequence, bool response, int error_code) {
    bool ret = false;
    response_validate_dms_msg_t resp;
    resp.sequence_no = sequence;
    resp.response = response;
    resp.err_code = error_code;
    do {
        if( false == send_msg( (generic_msg_t *)&resp, (msg_type_t)RESPONSE_VALIDATE_DMS, sizeof(resp), queue_name, queue_name, msg_id++ ) ) {
            LOG_E (TAG,"sending msg to response thread failed\n");
            break;
        }
        LOG_I(TAG, "response for validate dms command successful from main\n");
        ret = true;
    } while (false);
}

bool response_dms_calibration(response_dms_calibration_msg_t dms_calibration) {
    bool ret = false;
    do {
        if( false == send_msg( (generic_msg_t *)&dms_calibration, (msg_type_t)RESPONSE_DMS_CALIBRATION, sizeof(dms_calibration), queue_name, queue_name, msg_id++ ) ) {
            LOG_E (TAG,"sending msg to response thread failed\n");
            break;
        }
        LOG_I(TAG, "response for dms calibration command successful from main\n");
        ret = true;
    } while (false);

}

bool response_end_dms_session(int sequence, bool response) {
    bool ret = false;
    response_end_dms_session_msg_t resp;
    resp.sequence_no = sequence;
    resp.response = response;
    do {
        if( false == send_msg( (generic_msg_t *)&resp, (msg_type_t)RESPONSE_END_DMS_SESSION, sizeof(resp), queue_name, queue_name, msg_id++ ) ) {
            LOG_E (TAG,"sending msg to response thread failed\n");
            break;
        }
        LOG_I(TAG, "response for end dms session command successful from main\n");
        ret = true;
    } while (false);
}

bool response_toggle_annotation(int sequence, bool response) {
    bool ret = false;
    response_toggle_annotation_msg_t resp;
    resp.sequence_no = sequence;
    resp.response = response;
    do {
        if( false == send_msg( (generic_msg_t *)&resp, (msg_type_t)RESPONSE_TOGGLE_ANNOTATION, sizeof(resp), queue_name, queue_name, msg_id++ ) ) {
            LOG_E (TAG,"sending msg to response thread failed\n");
            break;
        }
        LOG_I(TAG, "response for toggle annotation command successful from main\n");
        ret = true;
    } while (false);
}

bool response_rotate_stream(int sequence, int error_code) {
    bool ret = false;
    response_rotate_stream_msg_t resp;
    resp.sequence_no = sequence;
    ...
    if( false == send_msg( (generic_msg_t *)&resp, (msg_type_t)RESPONSE_ROTATE_STREAM, sizeof(resp), queue_name, queue_name, msg_id++ ) ) {
        LOG_E (TAG,"sending msg to response thread failed\n");
    } else {
        LOG_I(TAG, "response for flip stream command successful from main\n");
        ret = true;
    }
    return ret;
}
...
CURLcode curl_fetch_url(CURL *ch, const char *url, struct curl_fetch_st *fetch) {
    long curl_conn_timeout_secs = 120L;
    long curl_max_timeout_secs = 1200L;
...
bool prepare_multipart_form_curl_request(const char *url, const char *path){
    CURL *ch;                                               /* curl handle */
    CURLcode rcode;                                         /* curl result code */
    ...
    json_object_set_new(root, "device_id", json_string(device_ssid.c_str()));
    ...
    string header_device_id = "X-DeviceId: " + device_ssid;
...
bool create_metadata_json_video_zip(start_dms_calibration_msg_t *dms_json_params) {

    int ret = 0;
    stringstream ss;

    string metadata_json_filename = base_file_path + base_file_name + dms_json_params->uuid + "_" + std::to_string(dms_json_params->timestamp) + ".json";
    string calibration_video_filename = base_file_path + base_file_name + dms_json_params->uuid + "_" + std::to_string(dms_json_params->timestamp) + ".mp4";
    string calibration_tar_filename = base_file_path + base_file_name + dms_json_params->uuid + "_" + std::to_string(dms_json_params->timestamp) + ".tar";
    string calibration_tar_filename_simple = base_file_name + dms_json_params->uuid + "_" + std::to_string(dms_json_params->timestamp) + ".tar";

    json_t *root = json_object();
    if (root == NULL) {
        LOG_E(TAG, "root json object creation failed");
        return false;
    }
    json_object_set_new(root, "SN", json_string((dms_json_params->sn)));
    json_object_set_new(root, "PN", json_string((dms_json_params->pn)));
    json_object_set_new(root, "uuid", json_string((dms_json_params->uuid)));
    json_object_set_new(root, "timestamp", json_integer(dms_json_params->timestamp));

    json_t *calibration_data = json_object();
    if (calibration_data == NULL) {
        LOG_E(TAG, "calibration_data json object creation failed");
        return false;
    }
    json_object_set_new(calibration_data, "yaw", json_real(final_dms_calibration_data.yaw));
    json_object_set_new(calibration_data, "pitch", json_real(final_dms_calibration_data.pitch));
    json_object_set_new(calibration_data, "roll", json_real(final_dms_calibration_data.roll));
    json_object_set_new(calibration_data, "yaw_var", json_real(final_dms_calibration_data.yaw_var));
    json_object_set_new(calibration_data, "pitch_var", json_real(final_dms_calibration_data.pitch_var));
    json_object_set_new(calibration_data, "roll_var", json_real(final_dms_calibration_data.roll_var));
    json_object_set_new(calibration_data, "face_width", json_real(final_dms_calibration_data.face_width));
    json_object_set_new(calibration_data, "face_height", json_real(final_dms_calibration_data.face_height));
    json_object_set_new(calibration_data, "face_width_var", json_real(final_dms_calibration_data.face_width_var));
    json_object_set_new(calibration_data, "face_height_var", json_real(final_dms_calibration_data.face_height_var));

    json_object_set_new(calibration_data, "calib_frame_cnt", json_integer(final_dms_calibration_data.calib_frame_cnt));
    json_object_set_new(calibration_data, "yaw_correction", json_real(final_dms_calibration_data.yaw_correction));
    json_object_set_new(calibration_data, "pitch_correction", json_real(final_dms_calibration_data.pitch_correction));
    json_object_set_new(calibration_data, "roll_correction", json_real(final_dms_calibration_data.roll_correction));
    json_object_set_new(calibration_data, "cam_distance_correction", json_real(final_dms_calibration_data.cam_distance_correction));
    json_object_set_new(calibration_data, "calib_status_code", json_integer(final_dms_calibration_data.calib_status_code));
    json_object_set_new(calibration_data, "calibration_feedback", json_string(dms_json_params->feedback));
    json_object_set_new(calibration_data, "flip_status", json_integer(dms_camera_flip));

    json_object_set_new(root, "calibration_data", calibration_data);

    ret = json_dump_file(root, metadata_json_filename.c_str(), JSON_INDENT(4)|JSON_REAL_PRECISION(4));
    if (ret) {
        LOG_E(TAG, "creating metadata json failed");
        return false;
    }

    if (file_is_present(dms_calibration_recording_file) == false) {
        LOG_E(TAG, "calibration video is not available");
        return false;
    }

    //rename calibration video file
    if (rename(dms_calibration_recording_file.c_str(), calibration_video_filename.c_str()) < 0)
        LOG_E(TAG, "moving calibration video file failed");

    //compress the contents(json and .mp4) in a TAR file
    ss.str("");
    ss << "tar -Pcvf " << calibration_tar_filename << " " << metadata_json_filename << " " << calibration_video_filename;
    if (system_execute("creating final tar file", ss.str()) != 0) {
        LOG_E(TAG, "tar file creation failed");
    }

    /* delete the json file */
    if (!file_delete(metadata_json_filename))
        LOG_E(TAG, "file delete failed for file: %s", metadata_json_filename.c_str());

    /* delete the calibration video file */
    if (!file_delete(calibration_video_filename))
        LOG_E(TAG, "file delete failed for file: %s", calibration_video_filename.c_str());

    /* Checks if base path exist. Else creates one. If failed to do so exit from here */
    if (!file_mkdir(base_folder_dms_calibration_data, 0777)) {
        LOG_E(TAG, "Failed to create base folder to keep calibration data");
        return false;
    }

    //move the tar file in base folder
    string new_base_path_tar_file = base_folder_dms_calibration_data + calibration_tar_filename_simple;
    if (rename(calibration_tar_filename.c_str(), new_base_path_tar_file.c_str()) < 0)
        LOG_E(TAG, "moving calibration tar file failed");
...
bool handle_dms_prerequisites(json_t *data) {
    bool ret = false;
    start_dms_calibration_msg_t dms_json_params;
    do {

        if (json_object_get(data, "uuid") == NULL && json_object_get(data, "timestamp") == NULL && json_object_get(data, "sn") == NULL) {
            LOG_E(TAG, "uuid, timestamp and sn not present in json");
            LOG_I(TAG, "not creating metadata json and video zip");
            ret = true;
            break;
        }
        ...
        string uuid = json_string_value(json_object_get(data, "uuid"));
        nd_strncpy(dms_json_params.uuid, uuid.c_str(), sizeof(dms_json_params.uuid));

        if(json_is_integer(json_object_get(data, "timestamp")) == false) {
            LOG_E(TAG, "timestamp not present in json");
            break;
        }
        dms_json_params.timestamp = json_integer_value(json_object_get(data, "timestamp"));
        ...
        nd_strncpy(dms_json_params.pn, pn.c_str(), sizeof(dms_json_params.pn));

        if(json_is_string(json_object_get(data, "SN")) == false) {
            LOG_E(TAG, "sn not present in json");
            break;
        }
        string sn = json_string_value(json_object_get(data, "SN"));
        nd_strncpy(dms_json_params.sn, sn.c_str(), sizeof(dms_json_params.sn));
        ...
        nd_strncpy(dms_json_params.feedback, notes_str.c_str(), sizeof(dms_json_params.feedback));

        /* This API will rename the metadata json and recorded video according to uuid and ts received from app and zip them */
        create_metadata_json_video_zip(&dms_json_params);
        ret = true;
    } while(false); 

    // deleting uuid and timestamp from json
    json_object_del(data, "uuid");
    json_object_del(data, "timestamp");
    json_object_del(data, "calibration_feedback");
...
bool handle_pair_prerequisites(json_t *data, string &accessory_type) {
    ...
    do {
        ...
        if (accessory_type == "DMS_CAMERA") {
            LOG_I(TAG, "dms prerequisites");
            if(!handle_dms_prerequisites(data)) {
                LOG_E(TAG, "dms prerequisites failed");
            }
        } else if (accessory_type == "DHUBX") {
...
void dms_usb_connect_cb(int32_t sig_type) {
	switch(sig_type) {
		case DMS_STATUS_CONNECTED:
			LOG_I(TAG, "DMS camera Connected\n");
			break;
		case DMS_STATUS_DISCONNECTED:
			LOG_I(TAG, "DMS camera Disconnected\n");
			break;
		default:
			LOG_I(TAG, "Unknown signal type: %d\n", sig_type);
			break;
	}
}

bool handle_validate_dms(string &pn, string &sn, int &err_code) {
    memset(&dmsCam, 0 , sizeof(dmsCam_t));
    memset(dmsCam_SN, 0 , sizeof(dmsCam_SN));
    memset(dmsCam_PN, 0 , sizeof(dmsCam_PN));

    dmsCam.args.devPath = "/dev/dms_h264";
    dmsCam.args.ir_level = DMS_IR_L1;
    dmsCam.args.width = dms_camera_width;
    dmsCam.args.height = dms_camera_height;
    dmsCam.args.fps = dms_camera_fps;
    dmsCam.args.bitrate = dms_camera_bitrate;
    dmsCam.args.gop = dms_camera_fps;
    ...
    // read dms_camera_flip from db
    prop_data_t prop_data;
    string dms_rotate_key = "CAM" + to_string(DMS_CAM) + "_ROTATE";
    if(!get_property_DB(dms_rotate_key, &prop_data)) { 
        LOG_E(TAG, "get_property_DB failed to get dms_rotate");
    }
    if (prop_data.value == "1") {
        dms_camera_flip = 1;
    } else {
        dms_camera_flip = 0;
    }
    ...
    if (!strcmp(sn.c_str(), (const char*)dmsCam_SN)) {
        LOG_E(TAG, "dms SN matches, which provided by App\n");
        return true;
    } else {
        LOG_E(TAG, "dms SN doesn't match, which provided by App %s\n", sn.c_str());
    }

/* May need PN number in future */
#if 0
    if (dmsGetPN(&dmsCam, dmsCam_PN, PN_LEN) < 0) {
        LOG_E(TAG, "dms PN Eeprom Get ERROR! \n");
        err_code = 1;
        return false;
    } else {
        dmsCam_PN[PN_LEN] = '\0';
        LOG_I(TAG, "dms PN Eeprom Verify %s\n", dmsCam_PN);
    }
    if (!strcmp(pn.c_str(), (const char*)dmsCam_PN)) {
        LOG_E(TAG, "dms PN matches, which provided by App\n");
        return true;
    } else {
        LOG_E(TAG, "dms PN doesn't match, which provided by App %s\n", pn.c_str());
    }
#endif
...
bool convert_nv12_to_jpeg(uint8_t* calib_image_data_nv12_ptr) {
    GstMapInfo map;
    GstBuffer *dst_buffer = gst_buffer_new_allocate(NULL, dms_analytics_width*dms_analytics_height*1.5, NULL);
    gst_buffer_map(dst_buffer, &map, GST_MAP_WRITE);
    memcpy((guchar *)map.data, calib_image_data_nv12_ptr, gst_buffer_get_size(dst_buffer));
    GstFlowReturn ret = gst_app_src_push_buffer((GstAppSrc*)elem_appsrc_analytics_calibration_image, dst_buffer);
    LOG_E(TAG, "elem_appsrc_analytics_calibration_image pushed buffer returned with %d", ret);
    if (ret == GST_FLOW_OK)
        return true;
    else
        return false;
...
bool handle_start_dms_calibration(response_dms_calibration_msg_t &dms_calibration) {
    if (currently_streaming == DMS_CAM) {
        app_an::start_calibration(dms_analytics_handle);
        is_calibration_started = true;

        struct timespec time_to_wait = {5, 0};
        struct timeval now;
        gettimeofday(&now, NULL);
        /* We will wait for max 10 sec to get the end_calibration callback from analytics */
        time_to_wait.tv_sec = now.tv_sec + 10;
        pthread_mutex_lock(&analytics_calibration_mutex);
        if (ETIMEDOUT == pthread_cond_timedwait(&analytics_calibration_end_cond, &analytics_calibration_mutex, &time_to_wait)) {
            LOG_E(TAG, "end calbration callback didn't come from analytics within 10 sec");
            is_calibration_started = false;
            pthread_mutex_unlock(&analytics_calibration_mutex);
            /* End the DMS streaming, in case of calibration timeout and return */
            stop_streaming(DMS_CAM);
            return false;
        } else {
            pthread_mutex_unlock(&analytics_calibration_mutex);
            is_calibration_started = false;
...
#ifdef TEST_DMS
        dms_json_params.uuid = "s3ed32452f3452";
        dms_json_params.timestamp = 123456790123;
        create_metadata_json_video_zip();
#endif

        bool res = convert_nv12_to_jpeg(final_dms_calibration_data.calib_image_data_nv12_ptr);
        if (!res) {
            LOG_E(TAG, "convert_nv12_to_jpeg has failed");
            return false;
        }

        gettimeofday(&now, NULL);
        /* We will wait for max 3 sec to get the base64 encoded data from appsink_analytics_jpeg_cb */
        time_to_wait.tv_sec = now.tv_sec + 3;
        pthread_mutex_lock(&analytics_calibration_image_mutex);
        if (ETIMEDOUT == pthread_cond_timedwait(&analytics_calibration_base64_cond, &analytics_calibration_image_mutex, &time_to_wait)) {
            LOG_E(TAG, "base64 encoded data didn't come from appsink_analytics_jpeg_cb within 3 sec");
            pthread_mutex_unlock(&analytics_calibration_image_mutex);
            /* End the DMS streaming, in case of calibration timeout and return */
            stop_streaming(DMS_CAM);
            return false;
        ...
        dms_calibration.yaw_correction = final_dms_calibration_data.yaw_correction;
        dms_calibration.pitch_correction = final_dms_calibration_data.pitch_correction;
        dms_calibration.roll_correction = final_dms_calibration_data.roll_correction;
        dms_calibration.cam_distance_correction = final_dms_calibration_data.cam_distance_correction;
        dms_calibration.calib_status_code = final_dms_calibration_data.calib_status_code;

        /* End the DMS streaming, once calibration is complete */
        stop_streaming(DMS_CAM);

        string dms_rotate_key = "CAM" + to_string(DMS_CAM) + "_ROTATE";
        string dms_rotate_str = to_string(dms_camera_flip);
        for (int i = 0; i < 5; i++) {
            if(!set_property_DB(dms_rotate_key, dms_rotate_str)) {
                LOG_E(TAG, "set_property_DB failed to set dms_rotate");
            } else {
                LOG_I(TAG, "set_property_DB success for dms_rotate");
                break;
            }
            sleep(2);
        }
        ...
        LOG_E(TAG, "dms is not streaming currently, so start calibration is not possible");
        return false;
    }
}

bool handle_rotate_stream(int camera_no, int &error_code) {
    bool ret = false;
    do {
        if(camera_no == DMS_CAM) {
            prop_data_t prop_data;
            int updated_dms_flip_state;
            if (dms_camera_flip == 1) {
                updated_dms_flip_state = 0;
            } else {
                updated_dms_flip_state = 1;
            }
            LOG_I(TAG, "*********************************************");
            LOG_I(TAG, "updated_dms_flip_state: %d", updated_dms_flip_state);
            LOG_I(TAG, "*********************************************");

            // flipping stream
            // error code 2 for rotate stream failure
            if (dmsCamSetVflip(&dmsCam, updated_dms_flip_state) < 0) {
                LOG_E(TAG, "Failed to set vertical flip");
                error_code = 2;
                break;
            }
            if (dmsCamSetHflip(&dmsCam, updated_dms_flip_state) < 0) {
                LOG_E(TAG, "Failed to set horizontal flip");
                error_code = 2;
                break;
            }
            dms_camera_flip = updated_dms_flip_state;
        } else {
            LOG_E(TAG, "rotate stream is not supported for camera no: %d", camera_no);
            error_code = 3;
            break;
        }
        ret = true;
...
int msg_loop(){
    ...
    while(1){
        ...
        case TEST_CONNECTION:
        {
           ...
           LOG_I(TAG, "dhubx toggle dhub mode: %d", appReqData.dhubx.toggle_dhub_mode);
        ...
        case STOP_STREAMING:
            ...
            command_sequence= g_msg->sequence_no;
            camera_no = g_msg->argument;
            if(camera_no >= EXT_CAM_1 && camera_no <= EXT_CAM_4) {
       ...
       case VALIDATE_DMS:
        {
            LOG_I(TAG, "validate dms case");
            validate_dms_msg_t *validate_dms = (validate_dms_msg_t *)m;

            int err_code = 0;
            string pn = validate_dms->pn;
            string sn = validate_dms->sn;

            bool ret = handle_validate_dms(pn, sn, err_code);
            response_validate_dms(validate_dms->sequence_no, ret, err_code);

            break;
        }

       case START_DMS_CALIBRATION:
       {
            LOG_I(TAG, "start dms calibration case");
            response_dms_calibration_msg_t resp_dms_calibration;

            bool ret = handle_start_dms_calibration(resp_dms_calibration);
            if (!ret) {
                LOG_E(TAG, "start dms calibration failed");
            }
            start_dms_calibration_msg_t *start_dms_calibration = (start_dms_calibration_msg_t *)m;

            resp_dms_calibration.sequence_no = start_dms_calibration->sequence_no;
            resp_dms_calibration.response = ret;

            if (start_dms_calibration->timestamp == 0 || start_dms_calibration->uuid == NULL || start_dms_calibration->sn == NULL) {
                LOG_E(TAG, "uuid, timestamp and sn not present in json not creating metadata json and video zip");
            } else {
                create_metadata_json_video_zip(start_dms_calibration);
            }

            response_dms_calibration(resp_dms_calibration);

            break;
       }

       case END_DMS_SESSION:
        {
            LOG_I(TAG, "end dms session case");
            bool ret = stop_streaming(DMS_CAM);
            //ret = handle_end_dms_session();
            if (!ret) {
                LOG_E(TAG, "end dms session failed");
            }

            response_end_dms_session(g_msg->sequence_no, ret);
            break;
        }

       case TOGGLE_ANNOTATION:
        {
            LOG_I(TAG, "toggle annotation case");

            toggle_annotation_msg_t *toggle_annotation = (toggle_annotation_msg_t *)m;
            int annotation_status = toggle_annotation->annotation_status;
            is_annotation_enabled = annotation_status;

            response_toggle_annotation(toggle_annotation->sequence_no, true);
            break;
        }
       case ROTATE_STREAM:
       {
            LOG_I(TAG, "rotate stream case");
            int error_code = 0;
            handle_rotate_stream(g_msg->argument, error_code);
            response_rotate_stream(g_msg->sequence_no, error_code);
            break;
       } 
...
bool receive_stop_led_ack(void* args) {
    ...
    while (true) {
        ...
        if (m->msg_type == RESP_INSTALLER_LED_BLINKING) {
            ...
            LOG_E(TAG, "Received unexpected message type %d while waiting for RESP_INSTALLER_LED_BLINKING", m->msg_type);
            delete msg;
        }
    }
    return ack_received;
...
void update_otacheck_state_file() {
    ...
    stringstream otacheck_state;
    otacheck_state << getpid();
    otacheck_state << " RUN_STATE";

    LOG_I(TAG, "Updated the otacheck_state file with %s", otacheck_state.str().c_str());
}

void *upload_dms_calibration_data(void *args)
{
    vector<string> vec;
    if (!get_files(base_folder_dms_calibration_data, vec)) {
        LOG_E(TAG, "Failed to get file details from given path %s", base_folder_dms_calibration_data.c_str());
        return NULL;
    }
    CIRCULAR_BUFFER_PATH = nd_device_obj->get_external_eMMC_mount_path();
    LOG_E(TAG, "circular buffer path coming as %s", CIRCULAR_BUFFER_PATH.c_str());

    while (1) {
        /* proceed to upload, only if internet is there in device */
        ...
        for (vector<string>::iterator iter = vec.begin(), end = vec.end(); iter != end; iter++) {
            ...
            /* code to parse the uuid from tar file name */
            while (((pos = s.find(delimiter)) != std::string::npos) && (count < 3)) {
                token = s.substr(0, pos);
                if (count == 2) {
                    LOG_I(TAG, "uuid found is %s", token.c_str());
                    break;
                }
            ...
            string res_url = base_upload_url_dms_calibration_data + token;
            string res_path = base_folder_dms_calibration_data + (*iter);
            LOG_I(TAG, "res_url: %s", res_url.c_str());
            LOG_I(TAG, "res_path: %s", res_path.c_str());

            if (token.length() != UUID_STR_LENGTH) {
                LOG_E(TAG, "uuid parsing had some issue for file %s, moving it to SDCARD for later reference", res_path.c_str());
                string final_dst_file = CIRCULAR_BUFFER_PATH + (*iter);
                if (!file_copy(res_path, final_dst_file))
                    LOG_E(TAG, "copy to SDcard failed");
                if (!file_delete(res_path))
                    LOG_E(TAG, "file delete failed");
            ...
            while (retry_attempt < 5) {
                bool ret = prepare_multipart_form_curl_request(res_url.c_str(), res_path.c_str());
                if (ret) {
                    is_upload_success = true;
                    /* delete the tar file, if upload succeeds */
                    if (!file_delete(res_path))
                        LOG_E(TAG, "file delete failed for file: %s", res_path.c_str());
                    break;
                }
                LOG_E(TAG, "upload failed for file %s", res_path.c_str());
                sleep(5);
                retry_attempt++;
            }
            if (!is_upload_success) {
                string final_dst_file = CIRCULAR_BUFFER_PATH + (*iter);
                LOG_E(TAG, "upload failed after 5 retrials for file: %s, moving it to SD card for later reference", res_path.c_str());
                if (!file_copy(res_path, final_dst_file))
                    LOG_E(TAG, "copy to SDcard failed");
                if (!file_delete(res_path))
                    LOG_E(TAG, "file delete failed");
            } else {
                LOG_I(TAG, "DMS calibration upload successful for file %s", res_path.c_str());
            }
        }
        break;
    }
...
int main (int argc, char **argv)
{
  ...
  nd_device_obj_init();
   ...
   if (file_is_present(dms_calibration_recording_file) == true) {
       LOG_I(TAG, "stale video file is present related to DMS, deleting it");
       /* delete the calibration video file */
      if (!file_delete(dms_calibration_recording_file))
          LOG_E(TAG, "file delete failed for file: %s", dms_calibration_recording_file.c_str());
   }

   pthread_t upload_dms_calibration_data_th;
   if (!pthread_create (&upload_dms_calibration_data_th, NULL, upload_dms_calibration_data, NULL)) {
       LOG_I (TAG, "Thread created for upload dms data");
   }
