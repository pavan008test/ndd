...
public:
    ...
    //Used for OBS file to generate meta data in genmeta class
	struct gps_metadata_t {
    ...
    //is using the same structure. Hence the need of below gps_sensor_data_data_t
    //struct
    struct gps_sensor_data_t {
        struct gps_data_t gps_data;
        uint64_t gps_index;
        uint64_t raw_time_micro;
        double altitudeMSL;
	...
	typedef bool gps_callback_t (gps_sensor_data_t gps_sensor_data);
