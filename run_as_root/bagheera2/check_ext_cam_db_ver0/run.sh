...
if [[ -f /home/ubuntu/.nddevice/ext_cam.db ]]; then
    ...
    sqlite3 /home/ubuntu/.nddevice/ext_cam.db "SELECT name FROM sqlite_master WHERE type='table' AND name='EXTCAM';" | grep "EXTCAM"
    status2=$?
    if [[ $status2 == 0 ]]; then
        log "'EXTCAM' table exists in 'ext_cam.db' database"
        sqlite3 /home/ubuntu/.nddevice/ext_cam.db "PRAGMA table_info(EXTCAM);" | grep "PULL"
        status3=$?

        if [[ $status3 != 0 ]]; then
            log "Adding 'PULL' column to 'EXTCAM' table"
            # If "PULL" column does not exist, add it with default value of 0
            sqlite3 /home/ubuntu/.nddevice/ext_cam.db "ALTER TABLE EXTCAM ADD COLUMN PULL INTEGER DEFAULT 0;"
            status4=$?
