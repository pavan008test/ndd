...
if [[ -f /lib/modules/4.9.140-tegra/kernel/drivers/bag2_vendor/ignition/gpio-ignition.ko ]]; then
    if [[ $(md5sum /lib/modules/4.9.140-tegra/kernel/drivers/bag2_vendor/ignition/gpio-ignition.ko 2>/dev/null | awk -F " " '{print $1}') != 7a5eebfaadbebf14ff8d17ce560f018a ]]; then
        log "md5sum not matched for gpio-ignition.ko, replacing..."
        copy_file -f "gpio-ignition.ko" -d ${DEST_DIR} -b /home/ubuntu/.nddevice/backup/ -m 7a5eebfaadbebf14ff8d17ce560f018a -p 644 -o root:root
    ...
    log "No ignition ko file present in location /lib/modules/4.9.140-tegra/kernel/drivers/bag2_vendor/ignition/. so,copying it...."
    copy_file -f "gpio-ignition.ko" -d ${DEST_DIR} -b /home/ubuntu/.nddevice/backup/ -m 7a5eebfaadbebf14ff8d17ce560f018a -p 644 -o root:root
...
rmmod gpio-ignition
sleep 1
/sbin/insmod /lib/modules/4.9.140-tegra/kernel/drivers/bag2_vendor/ignition/gpio-ignition.ko
