...
log "########Start of copying original nvpmodel_t186_backup.conf as nvpmodel_t186.conf  ######"
if [[ -f /etc/nvpmodel/nvpmodel_t186_backup.conf ]] ;then
        log "Copying the nvpmodel_t186_backup.conf as nvpmodel_t186.conf"
	cp -f /etc/nvpmodel/nvpmodel_t186_backup.conf /etc/nvpmodel/nvpmodel_t186.conf
else
	log "nvpmodel_t186_backup.conf is not present"
fi
...
if [[ $(cat /sys/devices/system/cpu/cpu1/online) -ne 0 || $(cat /sys/devices/system/cpu/cpu2/online) -ne 0 || $(cat /sys/devices/17000000.gp10b/devfreq/17000000.gp10b/max_freq) -ne 1122000000 ]]; then       status1=1
else
    status1=0
