...
md5sum_nvpmodel_system=$(md5sum /etc/nvpmodel/nvpmodel_t186.conf |awk '{print $1}')
md5sum_nvpmodel_t186=$(md5sum nvpmodel_t186.conf |awk '{print $1}')

log "==================Enabling Denver Cores and Assigning CPU affinities to ND Services=================="
log "===========checking md5sum of nvpmodel_org.conf and nvpmodel_t186.conf file==========="
if [[ "$md5sum_nvpmodel_org" == "$md5sum_nvpmodel_system" ]]; then
log "The nvpmodel_org.conf file matches the device's nvpmodel_t186.conf file, with the same md5sum $md5sum_nvpmodel_system"
else
log "The nvpmodel_org.conf file does not match the device's nvpmodel_t186.conf file, as they have different md5sums $md5sum_nvpmodel_system."
fi
log "============nvpmodel_t186.conf md5sum checks are done========="
...
log "============Copying the latest nvpmodel_t186.conf file to the /etc/nvpmodel/============"
if [[ "$md5sum_nvpmodel_system" == "$md5sum_nvpmodel_t186" ]]; then
       log "Already updated nvpmodel_t186.conf is there,So not copying"
       status1=0
else
	log "Taking the backup of the existing nvpmodel_t186.conf as nvpmodel_t186_backup.conf"
	cp -rf /etc/nvpmodel/nvpmodel_t186.conf /etc/nvpmodel/nvpmodel_t186_backup.conf
	log "Copying the latest nvpmodel_t186.conf file to /etc/nvpmodel/"
	copy_file -f nvpmodel_t186.conf -d /etc/nvpmodel/ -m $md5sum_nvpmodel_t186 -p 644 -o root:root
	status1=$?
fi
log "===========Copying the latest nvpmodel_t186.conf is done==========="
...
if [[ $(cat /sys/devices/system/cpu/cpu1/online) -ne 1 || $(cat /sys/devices/system/cpu/cpu2/online) -ne 1 || $(cat /sys/devices/17000000.gp10b/devfreq/17000000.gp10b/max_freq) -ne 1122000000 ]]; then
    status2=1
else
