...
function backup {
	...
	then
		...
		sudo cp -rf --preserve $original_file $backuppath
		return $?
...
function rollback {
	...
	then
		...
		sudo cp -f --preserve $backpath $destpath
		status5=$?
		if [ $status5 == 0 ]
		then 
...
function copy_file {
	...
	sudo chmod $permission $default_source_path/$file_name
		if [ $? == 0 ]
		then
	...
	then
		...
		sudo chown $ownership $default_source_path/$file_name
		if [ $? == 0 ]
		then
	...
	sudo cp -f --preserve $default_source_path/$file_name $dest_path
        elif [ $command_mode == "move" ] ; then
	sudo mv -f $default_source_path/$file_name $dest_path
        fi
	...
	then
		...
		rollback "$backup_path/$dest_filename" "$dest_path"
		return 1
	fi
...
sudo chown ubuntu:ubuntu /home/ubuntu/.nddevice/log/bhcopy.log
