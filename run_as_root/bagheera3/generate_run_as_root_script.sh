...
conf_file_path=$(pwd)/../../../device-build/run_as_root/bagheera3/

if [[ $1 == "bagheera3_US" ]]; then
conf_file=${conf_file_path}/run_as_root_bagheera3_US.conf
elif [[ $1 == "bagheera3_IN" ]]; then
conf_file=${conf_file_path}/run_as_root_bagheera3_IN.conf
else
conf_file=${conf_file_path}/run_as_root_bagheera3_US.conf
fi
...
## This below line is added to not to remove the backup files which are taking backup duing the RAR tasks, even when device reboot.
echo "rm -f /home/ubuntu/.nddevice/com_backup"  >>$OUTPUT
##run of each script
while read folder
do
...
##This line is added to restore back that file to get backup cleaned up during the reboot through crontab where it checks for this status file
echo "touch /home/ubuntu/.nddevice/com_backup"  >>$OUTPUT
...
s3cmd get s3://netradyne-sharing/b3_blackvideo/x.6.12/blackvideo_editing_dms_LD.mp4 out/copy_blackvideo/
fi
