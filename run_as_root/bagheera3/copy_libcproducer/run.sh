...
md5sum_libcproducer_system=$(md5sum /usr/local/lib/libcproducer.so |awk '{print $1}')

log "============ Copying the kinesis lib libcproducer.so file ==========="
log "checking md5sum of existing /usr/local/lib/libcproducer.so file"
...
status=$(copy_file -f libcproducer.so -d /usr/local/lib/ -b /home/ubuntu/.nddevice/backup -m $md5sum_libcproducer  -p 755 -o root:root)$?
