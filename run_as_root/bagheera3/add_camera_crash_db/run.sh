...
check_cam_crash_count() {
    if /bin/vendor/sqlite3 "$DB_PATH" "SELECT 1 FROM CAMERA_CRASH_DB WHERE PROPERTY LIKE 'CAM%_CRASH_COUNT' LIMIT 1;" | grep -q 1; then
...
if [ ! -f "$DB_PATH" ]; then
    ...
    if /bin/vendor/sqlite3 "$GENPROP_DB" "SELECT 1 FROM GENPROP WHERE PROPERTY LIKE 'CAM%_CRASH_COUNT' LIMIT 1;" | grep -q 1; then
