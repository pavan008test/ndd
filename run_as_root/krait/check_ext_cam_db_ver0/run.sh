...
if [[ -f /home/ubuntu/.nddevice/db/ext_cam.db ]]; then

    log "ext_cam.db exists in /home/ubuntu/.nddevice/db/"

    if [[ -f /home/ubuntu/.nddevice/backup/ext_cam.db ]]; then
        log "Already ext_cam.db backup is done"
    else
        backup /home/ubuntu/.nddevice/db/ext_cam.db /home/ubuntu/.nddevice/backup/
        status1=$?
        log "Backed up ext_cam.db"
    fi

    # Check if EXTCAM table exists
    sqlite3 /home/ubuntu/.nddevice/db/ext_cam.db "SELECT name FROM sqlite_master WHERE type='table' AND name='EXTCAM';" | grep "EXTCAM"
    status2=$?
    if [[ $status2 == 0 ]]; then
        log "'EXTCAM' table exists in 'ext_cam.db' database"
        sqlite3 /home/ubuntu/.nddevice/db/ext_cam.db "PRAGMA table_info(EXTCAM);" | grep "PULL"
        status3=$?

        if [[ $status3 != 0 ]]; then
            log "Adding 'PULL' column to 'EXTCAM' table"
            # If "PULL" column does not exist, add it with default value of 0
            sqlite3 /home/ubuntu/.nddevice/db/ext_cam.db "ALTER TABLE EXTCAM ADD COLUMN PULL INTEGER DEFAULT 0;"
            status4=$?

            if [[ $status4 != 0 ]]; then
                    log "Failed to add 'PULL' column to 'EXTCAM' table, deleting ext_cam.db"
                    rm /home/ubuntu/.nddevice/db/ext_cam.db
                    final_status=$?
