...
log "Waiting max 8 min for run_as_root service to complete its task"
timeout -t 480 sh -c 'while : ; do if [ -n "`grep "run_as_root" /home/ubuntu/.nddevice/ota_temp/run_as_root/run_as_root_status`" ] ; then break; else sleep 2; continue; fi ; done' && echo "run_as_root success"
...
/usr/bin/nd_service.sh -c stop -n run_as_root -p /home/ubuntu/.nddevice/__version__/service/
/usr/bin/nd_service.sh -c clean -n run_as_root -p /home/ubuntu/.nddevice/__version__/service/
exit 1
else
