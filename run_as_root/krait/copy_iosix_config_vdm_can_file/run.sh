...
if [[ -s /data/nd_files/config/vdm_can_adapter.ini ]]
then 
log "vdm_can_adapter.ini file already exist and not empty. So not copying...."
status1=0
else
status1=$(copy_file -f vdm_can_adapter.ini -d /data/nd_files/config/ -m $CHECKSUM_vdm_can_adapter)
fi

log "=====Checks to copy iosix_config.ini file=========="
if [[ -s /data/nd_files/config/iosix_config.ini ]]
then 
log "iosix_config.ini file already exist and not empty. So not copying...."
status2=0
else
status2=$(copy_file -f iosix_config.ini -d /data/nd_files/config/ -m $CHECKSUM_iosix_config)
fi
