...
//     const ble_accessory_data_64_t *accessory_msg = reinterpret_cast<ble_accessory_data_64_t *>(msg);
...
bool IndicateInstallerActivity(const char *src, const char *dest, std::atomic<int> &msg_id) {
    ...
    generic_msg_64_t msg{};

    if (send_msg(&msg, INSTALLER_SCAN_INDICATE, sizeof(generic_msg_64_t), src, dest, ++msg_id)) {
...
bool FetchWriteCharData(void *msg, std::string &mac_out, std::string &uuid_out, std::vector<std::string> &data_out) {
    ...
    const bt_device_write_char_msg_64_t *bt_device_write_char_msg = reinterpret_cast<bt_device_write_char_msg_64_t *>(msg);
...
std::string FetchMsgClientName(void *msg) {
    const generic_msg_64_t *generic_msg = reinterpret_cast<generic_msg_64_t *>(msg);
...
bool FetchIgnitionStatus(void *msg, ignition_status_t &ign_status, bool &is_wakeup_out) {
    ...
    powermon_ignition_msg_64_t *ignition_status_msg = reinterpret_cast<powermon_ignition_msg_64_t *>(msg);

    // Correcting values for crank_change_time, ignition_status, and lpw_status
    // due to incorrect data caused by structure alignment issues in the received message.
    // These fields are being manually set here to ensure data is correct.

    ignition_status_msg->crank_change_time = get_system_time();
    ignition_status_msg->status = nd_device_obj->get_crank_level();
...
bool FetchSpeedEventUpdate(void *msg, int &handle, int &speed) {
    ...
    const res_speed_update_msg_64_t *speed_update_msg = reinterpret_cast<res_speed_update_msg_64_t *>(msg);
...
bool FetchIdleEventUpdate(void *msg, int &handle, bool &idle_on) {
    ...
    const res_idle_update_msg_64_t *idle_update_msg = reinterpret_cast<res_idle_update_msg_64_t *>(msg);
...
bool SendDeviceWriteCharResponse(bool write_status, BTErrCode err_code, const char *src, const char *dest,
    ...
    bt_device_write_char_response_64_t resp{};
    resp.response = write_status;
    resp.code = err_code;

    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&resp), DEVICE_WRITE_CHAR_RESPONSE,
                 sizeof(bt_device_write_char_response_64_t), src, dest, ++msg_id)) {
...
bool SendCreateHotspotMsg(bool enable, int band_type, const std::string &addr, const char *src, const char *dest,
    ...
    create_installer_hotspot_64_t msg{};
    msg.enable = enable;
    msg.band_type = band_type;

    constexpr size_t kMacAddressLength = 18;
    nd_strncpy(msg.mac_addr, addr.c_str(), kMacAddressLength);

    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), CREATE_INSTALLER_HOTSPOT,
                 sizeof(create_installer_hotspot_64_t), src, dest, ++msg_id)) {
        LOG_I(kLogTag, "Sent create hotspot message");
        status = true;
    } else {
        LOG_E(kLogTag, "Sending create hotspot message failed");
    }
...
bool SendDriverLoginAudioNotification(const std::string &file, const char *src, const char *dest,
                                      std::atomic<int> &msg_id) {
    bool status = false;
    driver_login_audio_notify_msg_64_t msg{};

    nd_strncpy(msg.file, file.c_str(), FNAME_LEN);

    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), DRIVER_LOGIN_AUDIO_NOTIFY,
                 sizeof(driver_login_audio_notify_msg_64_t), src, dest, ++msg_id)) {
...
bool SendDriverLoginQueryResponse(std::vector<std::string> ids, std::vector<std::string> timestamps,
    ...
    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&resp), RES_DRV_LOGIN_QUERY,
                 sizeof(resp_drv_login_query_64), src, dest, ++msg_id)) {
...
bool SendPrivacyModeToObservers(bool privacy_mode, const char *src, const char *dest, std::atomic<int> &msg_id) {
    ...
    privacy_mode_update_msg_64_t msg{};
    msg.privacy_on = privacy_mode;

    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), PRIVACY_MODE_UPDATE, sizeof(privacy_mode_update_msg_64_t),
                 src, dest, ++msg_id)) {
...
//     driveri_app_login_update_msg_64_t msg{};
//     msg.status = login;

//     if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), DRIVER_LOGIN_UPDATE, sizeof(driveri_app_login_update_msg_64_t),
//                  src, dest, ++msg_id)) {
...
bool SendDriverIdsToObservers(std::vector<std::string> ids, std::vector<std::string> timestamps,
    ...
    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), DRVID_UPDATE, sizeof(drv_id_update_msg_64_t),
                 src, dest, ++msg_id)) {
...
bool FetchIdleEventRegistrationResponse(void *msg, int &handle, idle_reg_type_t &type) {
    bool status = false;
    const res_idle_reg_msg_64_t *idle_reg_msg = reinterpret_cast<res_idle_reg_msg_64_t *>(msg);
...
bool SendIdleRegistrationRequest(int speed, int secs, idle_reg_type_t type,
                                 const char *src, const char *dest, std::atomic<int> &msg_id) {
    bool status = false;
    req_idle_reg_msg_64_t req_msg{};

    req_msg.speed = speed;
    req_msg.idle_secs = secs;
    req_msg.reg_type = type;

    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&req_msg), REQ_IDLE_REG, sizeof(req_idle_reg_msg_64_t),
                 src, dest, ++msg_id)) {
        status = true;
...
bool FetchSpeedEventRegistrationResponse(void *msg, int &handle, speed_reg_type_t &type) {
    bool status = false;
    const res_speed_reg_msg_64_t *speed_reg_msg = reinterpret_cast<res_speed_reg_msg_64_t *>(msg);
...
bool SendSpeedRegistrationRequest(int speed, int secs, speed_reg_type_t type,
                                 const char *src, const char *dest, std::atomic<int> &msg_id) {
    bool status = false;
    req_speed_reg_msg_64_t req_msg{};

    req_msg.speed = speed;
    req_msg.contig_secs = secs;
    req_msg.reg_type = type;

    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&req_msg), REQ_SPEED_REG, sizeof(req_speed_reg_msg_64_t),
                 src, dest, ++msg_id)) {
        status = true;
        LOG_I(kLogTag, "Register with speed service for speed event %d", static_cast<int32_t>(type));
    } else {
        LOG_E(kLogTag, "Sending speed registration msg failed");
    }
    return status;
}
...
bool SendFrStatusToObservers(bool fr_status, const char *src, const char *dest, std::atomic<int> &msg_id) {
    bool status = false;
    driver_login_fr_update_msg_64_t msg{};
    msg.status = fr_status;
    // get system time in 32 bit params
    get_system_time_32(&msg.timestamp1, &msg.timestamp2);
    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), DRIVER_LOGIN_UPDATE, sizeof(driver_login_fr_update_msg_64_t),
                 src, dest, ++msg_id)) {
        status = true;
        LOG_I(kLogTag, "Sent fr status %d to observer", fr_status);
    } else {
        LOG_E(kLogTag, "Sending fr status to observer failed");
    }
    return status;
}
...
bool SendUnregisterSpeedEventHandle(int handle, const char *src, const char *dest, std::atomic<int> &msg_id) {
    bool status = false;
    req_speed_unreg_msg_64_t req_msg{};

    req_msg.handle = handle;

    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&req_msg), REQ_SPEED_UNREG, sizeof(req_speed_unreg_msg_64_t),
                 src, dest, ++msg_id)) {
        status = true;
...
bool SendUnregisterIdleEventHandle(int handle, const char *src, const char *dest, std::atomic<int> &msg_id) {
    bool status = false;
    req_idle_unreg_msg_64_t req_msg{};

    req_msg.handle = handle;

    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&req_msg), REQ_IDLE_UNREG, sizeof(req_idle_unreg_msg_64_t),
                 src, dest, ++msg_id)) {
        status = true;
...
bool SendEngineIdleStatusToObservers(bool idle, const char *src, const char *dest, std::atomic<int> &msg_id) {
    ...
    eng_idle_update_msg_64_t msg{};
    msg.idle_on = idle;

    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), ENG_IDLE_UPDATE, sizeof(eng_idle_update_msg_64_t),
                 src, dest, ++msg_id)) {
...
bool SendGetIgnitionStatus(const char *src, const char *dest, std::atomic<int> &msg_id) {
    ...
    generic_msg_64_t msg{};
    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), GET_POWERMON_IGNITION_STATUS, sizeof(msg),
                 src, dest, ++msg_id)) {
...
bool SendAntennaTimeResponse(const char *src, const char *dest, std::atomic<int> &msg_id) {
    ...
    generic_msg_64_t msg{};
    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), REQUEST_ANTENNA_TIME_RESPONSE, sizeof(msg),
                 src, dest, ++msg_id)) {
...
bool SendDriverLoginUpdate(bool login_status, const char *src, const char *dest, std::atomic<int> &msg_id) {
    ...
    driveri_app_login_update_msg_64_t msg{};
    msg.status = login_status;

    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), DRIVER_LOGIN_UPDATE, sizeof(driveri_app_login_update_msg_64_t),
                 src, dest, ++msg_id)) {
        LOG_I(kLogTag, "Sent DRIVER_LOGIN_UPDATE to observers");
        status = true;
    } else {
        LOG_E(kLogTag, "Sending DRIVER_LOGIN_UPDATE to observers failed");
    }
...
bool SendGenericMessage(msg_type_t type, const char *msg_name, const char *src, const char *dest, std::atomic<int> &msg_id) {
    ...
    if (send_msg(&msg, type, sizeof(generic_msg_64_t), src, dest, ++msg_id)) {
...
bool SendInternalEventMessage(const char *msg_name, const char* q_name, std::atomic<int> &msg_id, int32_t event) {
    bool status = false;
    nd_bt_internal_event_64_t msg{};

    msg.event_ = event;

    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), BT_INTERNAL_EVENT,
                 sizeof(nd_bt_internal_event_64_t), q_name, q_name, ++msg_id)) {
        LOG_I(kLogTag, "Sent %s message", msg_name);
        status = true;
    } else {
        LOG_E(kLogTag, "Sending %s message failed", msg_name);
...
bool FetchInternalEventMessage(void *msg, int32_t &event) {
    ...
    const nd_bt_internal_event_64_t *internal_event_msg = reinterpret_cast<nd_bt_internal_event_64_t *>(msg);
...
bool SendUserAlertMessage(int button, uint64_t tstamp, const std::string &alert_src,
    ...
    user_alert_msg_64_t msg{};
    msg.button = button;
    msg.timestamp1 = tstamp >> 32;
    msg.timestamp2 = tstamp & 0xFFFFFFFF;

    nd_strncpy(msg.source, alert_src.c_str(), sizeof(msg.source));

    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), USER_ALERT, sizeof(user_alert_msg_64_t), src, dest, ++msg_id)) {
...
bool SendVbusAvailabilityMessage(bool detected, const char *src, const char *dest, std::atomic<int> &msg_id) {
    ...
    vbus_bt_availability_64_t msg{};
    msg.bt_available = detected;

    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), VBUS_BT_AVAILABILITY, sizeof(vbus_bt_availability_64_t),
                 src, dest, ++msg_id)) {
...
bool SendAlertBeaconPairedMessage(const char *src, const char *dest, std::atomic<int> &msg_id) {
    bool status = false;
    generic_msg_64_t msg{};
    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), BLE_ALERT_DEVICE_PAIRED, sizeof(msg), src, dest,
                ++msg_id) ) {
...
bool SendLedBlinkMessage(const led_blink_status_t led_status, const uint16_t timeout_sec,
    ...
    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), INSTALLER_SCAN_INDICATE, sizeof(led_blink_req_msg_64_t),
                 src, dest, ++msg_id)) {
        status = true;
    } else {
...
bool SendStartQRScanMsg(const std::vector<std::string> &qr_login_tags, const char* src,
                        const char* dest, std::atomic<int> &msg_id) {
    bool status = false;
    req_qr_login_scan_msg_64_t msg{};
    ...
    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), START_QR_SCAN_REQ, sizeof(req_qr_login_scan_msg_64_t),
                                                      src, dest, ++msg_id) ) {
...
bool SendStopQRScanMsg(const char* src, const char* dest, std::atomic<int> &msg_id) {
    ...
    if (send_msg(reinterpret_cast<generic_msg_64_t *>(&msg), STOP_QR_SCAN_REQ, sizeof(generic_msg_64_t),
                                                      src, dest, ++msg_id) ) {
...
bool FetchQRScanStartStatus(void *msg, QrScanStatusCodes &status_code, std::string &reason) {
    ...
    const resp_qr_login_scan_status_msg_64_t *qr_scan_status_msg = reinterpret_cast<resp_qr_login_scan_status_msg_64_t *>(msg);
...
NDService* CreateServiceObj(const std::string &name, std::function<void(int signum)> sig_usr_fn_cb) {
    ...
    return NDService::get_service_obj(name, is_64_bit, std::move(sig_usr_fn_cb));
}
