...
bool PersistenceDB::Close() {
    ...
    if (db_) {
        ...
        // already null
        LOG_I(kLogTag, "DB already NULL: %s", db_path_.c_str());
    }
...
bool PersistenceDB::IntegrityCheck(bool delete_on_failure) {
    bool status = false;
    const char * const command = "PRAGMA integrity_check;";
    const auto ret_pair = ExecuteCommand(command, nullptr, nullptr);
    if (SQLITE_OK != ret_pair.first) {
        ...
        if (delete_on_failure) {
            ...
            if (boost::filesystem::remove(db_path_, ec)) {
                LOG_I(kLogTag, "File: %s deleted", db_path_.c_str());
            } else {
                LOG_E(kLogTag, "File: %s delete failed, ec: %s:%d", db_path_.c_str(), ec.message().c_str(), ec.value());
            }
        }
    } else {
        status = true;
