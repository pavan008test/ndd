...
static void handle_sm_err( service_mon_err_msg_t *msg ) {
    ...
    LOG_E(TAG, "Service error: %s : %ld : %d : %d : %s", msg->sname, msg->time, \
            msg->err_code, msg->err_code_aux, msg->err_msg);

    //Add the error log to json
    add_err_log(msg->time, msg->sname, msg->err_code, msg->err_code_aux, msg->err_msg);   
}
...
static void handle_sm_start( service_mon_start_stop_msg_t *msg ) {
    ...
    LOG_I(TAG, "Service started: %s : %ld", msg->sname, msg->time );
}
...
static void handle_sm_stop( service_mon_start_stop_msg_t *msg ) {
    ...
    LOG_I(TAG, "Service stopped: %s : %ld", msg->sname, msg->time );
}
...
int main() {
    ...
    bool status_log = log_init( log_dir.c_str() );
    if(status_log == false) {
