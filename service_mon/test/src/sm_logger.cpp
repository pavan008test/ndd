 ...
 */
static bool update_json(json_t *root, int code, int aux_code, uint64_t ts, string pname, json_t *json_obj) {
    if( root == NULL ) {
    ...
    json_array_foreach(root, index, value_obj) {
        ...
        uint64_t ts_value_obj   = json_integer_value(json_object_get(value_obj, KEY_TS.c_str()));
        ts_value_obj            = ts_value_obj / ONE_MIN_MS;
        string pname_value_obj  = json_string_value(json_object_get(value_obj, KEY_PNAME.c_str()));
        ...
        if(tmp_code == code && tmp_aux_code == aux_code && \
                ts_value_obj == ts && pname_value_obj == pname ) {
...
bool add_err_log(uint64_t timestamp, string process_name, int code, int code_aux, string desc) {
    ...
    bool ret = update_json(root_json_data, code, code_aux, timestamp, process_name, json_obj);
    if( ret == false ) {
